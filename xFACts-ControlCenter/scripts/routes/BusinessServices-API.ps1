...
Add-PodeRoute -Method Post -Path '/api/business-services/redaction-search' -Authentication 'ADLogin' -ScriptBlock {
    if ((Test-ActionEndpoint -WebEvent $WebEvent) -eq $false) { return }
    try {
        $body = $WebEvent.Data
        $mode = $body.mode
...
Add-PodeRoute -Method Post -Path '/api/business-services/redaction-apply' -Authentication 'ADLogin' -ScriptBlock {
    if ((Test-ActionEndpoint -WebEvent $WebEvent) -eq $false) { return }
    try {
        $body = $WebEvent.Data
