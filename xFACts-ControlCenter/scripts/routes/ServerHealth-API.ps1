<#
.SYNOPSIS
    Registers the Server Health API endpoints.

.DESCRIPTION
    Registers the read and action API routes backing the Server Health page.
    Endpoints expose the active server list, GlobalConfig thresholds, live
    per-server memory / connection / activity / info / disk metrics, metric
    trends, availability group status and replica detail, Extended Events
    activity and per-category detail, live CPU gauges, and a zombie-connection
    kill action. xFACts-stored data and Extended Events history are read from
    the xFACts database on the availability group listener via the shared query
    wrappers; live per-server DMV and master queries run against each target
    instance with Invoke-Sqlcmd.

.COMPONENT
    ServerOps.ServerHealth

.NOTES
    File Name : ServerHealth-API.ps1
...
<# ============================================================================
   ROUTE: API ENDPOINTS
   ----------------------------------------------------------------------------
   The /api/server-health/* read endpoints plus the kill-zombies action and the
   shared /api/servers and /api/config/thresholds lookups. Every endpoint gates
   on Test-ActionEndpoint and returns its payload via Write-PodeJsonResponse.
   Listener-database reads use Invoke-XFActsQuery; live per-server DMV and
   master queries use Invoke-Sqlcmd against the selected instance.
   Prefix: (none)
   ============================================================================ #>
...
WHERE module_name = 'ServerOps'
  AND category = 'Activity_DMV'
  AND setting_name LIKE 'threshold_%'
  AND is_active = 1
"@
...
Add-PodeRoute -Method Get -Path '/api/server-health/connections' -Authentication 'ADLogin' -ScriptBlock {
    ...
    try {
        $serverName = $WebEvent.Query['server']; if (-not $serverName) { $serverName = 'AVG-PROD-LSNR' }
        $configRows = Invoke-XFActsQuery -Query @"
SELECT setting_name, CAST(setting_value AS INT) AS setting_value
FROM dbo.GlobalConfig
WHERE module_name = 'ServerOps'
  AND category = 'Activity_DMV'
  AND setting_name IN ('threshold_zombie_idle_minutes', 'threshold_open_trans_idle_minutes')
  AND is_active = 1
"@

        $zombieThreshold = 60
        $openTransIdleThreshold = 5
        foreach ($row in $configRows) {
            if ($row['setting_name'] -eq 'threshold_zombie_idle_minutes') { $zombieThreshold = [int]$row['setting_value'] }
            if ($row['setting_name'] -eq 'threshold_open_trans_idle_minutes') { $openTransIdleThreshold = [int]$row['setting_value'] }
        }

        $rows = Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query @"
DECLARE @zombie_threshold_minutes INT = $([int]$zombieThreshold);
DECLARE @open_trans_idle_minutes INT = $([int]$openTransIdleThreshold);
...
SELECT CAST(setting_value AS INT) AS setting_value
FROM dbo.GlobalConfig
WHERE module_name = 'ServerOps'
  AND category = 'Activity_DMV'
  AND setting_name = 'threshold_zombie_idle_minutes'
  AND is_active = 1
"@
        $zombieThreshold = if ($thresholdRows -and $thresholdRows.Count -gt 0) { [int]$thresholdRows[0]['setting_value'] } else { 60 }

        $webhookRows = Invoke-XFActsQuery -Query @"
SELECT webhook_url
FROM Teams.WebhookConfig
WHERE webhook_name = 'xFACts_Alerts'
  AND is_active = 1
"@
        $webhookUrl = if ($webhookRows -and $webhookRows.Count -gt 0) { $webhookRows[0]['webhook_url'] } else { $null }

        $zombieRows = Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query @"
SELECT session_id
FROM sys.dm_exec_sessions
WHERE program_name = 'Microsoft JDBC Driver for SQL Server'
  AND status = 'sleeping'
  AND open_transaction_count = 0
  AND DATEDIFF(MINUTE, last_request_end_time, GETDATE()) > $([int]$zombieThreshold)
"@ -QueryTimeout 10 -TrustServerCertificate -ApplicationName 'xFACts Control Center'

        $killedCount = 0; $errors = @()
        foreach ($row in $zombieRows) {
            $spid = [int]$row.session_id
            try {
                Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query "KILL $spid" -QueryTimeout 5 -TrustServerCertificate -ApplicationName 'xFACts Control Center'
                $killedCount++
        ...
        if ($killedCount -gt 0 -and $webhookUrl) {
            ...
            $zombieEmoji = [char]::ConvertFromUtf32(0x1F9DF)
            $gunEmoji = [char]::ConvertFromUtf32(0x1F52B)
            $skullEmoji = [char]::ConvertFromUtf32(0x1F480)
            ...
            $errorMessage = if ($teamsResult.Success) { [DBNull]::Value } else { $teamsResult.Error }
            Invoke-XFActsNonQuery -Query @"
INSERT INTO Teams.AlertQueue (source_module, alert_category, title, message, color, status, retry_count, trigger_type, trigger_value, created_dttm, processed_dttm, error_message)
VALUES ('ControlCenter', 'INFO', @title, @message, 'good', @status, @retryCount, 'ZombieKill', @serverName, GETDATE(), GETDATE(), @errorMessage)
"@ -Parameters @{
                title = $title
                message = $message
                status = $alertStatus
...
Add-PodeRoute -Method Get -Path '/api/server-health/open-transactions' -Authentication 'ADLogin' -ScriptBlock {
    ...
    try {
        ...
        $idleRows = Invoke-XFActsQuery -Query @"
SELECT CAST(setting_value AS INT) AS setting_value
FROM dbo.GlobalConfig
WHERE module_name = 'ServerOps'
  AND category = 'Activity_DMV'
  AND setting_name = 'threshold_open_trans_idle_minutes'
  AND is_active = 1
"@
        $idleThreshold = if ($idleRows -and $idleRows.Count -gt 0) { [int]$idleRows[0]['setting_value'] } else { 5 }

        $rows = Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query @"
SELECT s.session_id, s.login_name, s.program_name, s.host_name, ISNULL(DB_NAME(s.database_id), 'N/A') AS database_name,
    DATEDIFF(MINUTE, s.last_request_end_time, GETDATE()) AS idle_minutes, s.open_transaction_count
FROM sys.dm_exec_sessions s
WHERE s.session_id > 50 AND s.is_user_process = 1 AND s.open_transaction_count > 0 AND s.status = 'sleeping'
  AND DATEDIFF(MINUTE, s.last_request_end_time, GETDATE()) > $([int]$idleThreshold)
...
Add-PodeRoute -Method Get -Path '/api/server-health/trend' -Authentication 'ADLogin' -ScriptBlock {
    ...
    try {
        ...
        switch ($metric) {
            'ple' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, AVG(CAST(ple_seconds AS FLOAT)) AS value FROM ServerOps.Activity_DMV_Memory WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, ple_seconds AS value FROM ServerOps.Activity_DMV_Memory WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            'buffer_cache' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, AVG(buffer_cache_hit_ratio) AS value FROM ServerOps.Activity_DMV_Memory WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, buffer_cache_hit_ratio AS value FROM ServerOps.Activity_DMV_Memory WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            'memory_grants' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, AVG(CAST(memory_grants_pending AS FLOAT)) AS value FROM ServerOps.Activity_DMV_Memory WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, memory_grants_pending AS value FROM ServerOps.Activity_DMV_Memory WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            'zombie_count' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, AVG(CAST(zombie_count AS FLOAT)) AS value FROM ServerOps.Activity_DMV_ConnectionHealth WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, zombie_count AS value FROM ServerOps.Activity_DMV_ConnectionHealth WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            'connections' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, AVG(CAST(user_connections AS FLOAT)) AS value FROM ServerOps.Activity_DMV_Workload WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, user_connections AS value FROM ServerOps.Activity_DMV_Workload WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            'jdbc_connections' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, AVG(CAST(jdbc_total AS FLOAT)) AS value FROM ServerOps.Activity_DMV_ConnectionHealth WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, jdbc_total AS value FROM ServerOps.Activity_DMV_ConnectionHealth WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            'blocked_sessions' {
                if ($useAggregation) {
                    $query = "SELECT DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) AS timestamp, MAX(blocked_session_count) AS value FROM ServerOps.Activity_DMV_Workload WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) GROUP BY DATEADD(HOUR, DATEDIFF(HOUR, 0, snapshot_dttm), 0) ORDER BY timestamp"
                } else {
                    $query = "SELECT snapshot_dttm AS timestamp, blocked_session_count AS value FROM ServerOps.Activity_DMV_Workload WHERE server_name = @server AND snapshot_dttm >= DATEADD(HOUR, -@hours, GETDATE()) ORDER BY snapshot_dttm"
                }
            }
            default {
                if ($useAggregation) {
        ...
        $rows = Invoke-XFActsQuery -Query $query -Parameters @{ server = $serverName; hours = $hours }
        $results = @()
        foreach ($row in $rows) {
...
"@ -QueryTimeout 15 -TrustServerCertificate -ApplicationName 'xFACts Control Center'
...
Add-PodeRoute -Method Get -Path '/api/server-health/ag-status' -Authentication 'ADLogin' -ScriptBlock {
    ...
    try {
        ...
        $agClusterRows = Invoke-XFActsQuery -Query @"
SELECT ag_cluster_name
FROM dbo.ServerRegistry
WHERE server_name = @serverParam AND ag_cluster_name IS NOT NULL
"@ -Parameters @{ serverParam = $serverName }
        $agCluster = if ($agClusterRows -and $agClusterRows.Count -gt 0) { $agClusterRows[0]['ag_cluster_name'] } else { $null }
        ...
        $agServerRows = Invoke-XFActsQuery -Query @"
SELECT server_id, server_name
FROM dbo.ServerRegistry
WHERE ag_cluster_name = @agCluster
  AND server_type = 'SQL_SERVER'
ORDER BY server_id
"@ -Parameters @{ agCluster = $agCluster }

        $agServers = @()
        foreach ($row in $agServerRows) {
        ...
        $agRows = Invoke-Sqlcmd -ServerInstance $primaryServer -Database 'master' -Query @"
...
Add-PodeRoute -Method Get -Path '/api/server-health/ag-replica-detail' -Authentication 'ADLogin' -ScriptBlock {
    ...
    try {
        ...
        $replicaRows = Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query @"
SELECT
    ag.name AS ag_name,
...
"@ -QueryTimeout 10 -TrustServerCertificate -ApplicationName 'xFACts Control Center'
        ...
        $databaseRows = Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query @"
SELECT
...
"@ -QueryTimeout 10 -TrustServerCertificate -ApplicationName 'xFACts Control Center'
        ...
        $pleRows = Invoke-Sqlcmd -ServerInstance $serverName -Database 'master' -Query @"
...
"@ -QueryTimeout 10 -TrustServerCertificate -ApplicationName 'xFACts Control Center'
        ...
        $replicaInfo = $null
        if ($replicaRows) {
            $row = $replicaRows | Select-Object -First 1
        ...
        $ple = $null
        if ($pleRows) {
            $pleRow = $pleRows | Select-Object -First 1
            $ple = [long]$pleRow.ple
        }
...
"@ -Parameters @{ serverParam = $serverName; minutesParam = [int]$minutes }
...
Add-PodeRoute -Method Get -Path '/api/server-health/blocking-victims' -Authentication 'ADLogin' -ScriptBlock {
    ...
    try {
        ...
        $blockerSpidParam = if ($blockerSpid) { [int]$blockerSpid } else { [DBNull]::Value }
        $rows = Invoke-XFActsQuery -Query @"
DECLARE @cutoff DATETIME = DATEADD(MINUTE, -@minutesParam, GETDATE());
...
ORDER BY event_timestamp DESC
"@ -Parameters @{ serverParam = $serverName; minutesParam = [int]$minutes; blockerSpidParam = $blockerSpidParam }
        $results = @()
        foreach ($row in $rows) {
            $results += [PSCustomObject]@{
...
"@
        ...
        foreach ($row in $serverRows) {
            ...
            $cpuPct = $null
            try {
                $cpuRows = Invoke-Sqlcmd -ServerInstance $srvName -Database 'master' -Query @"
SELECT TOP 1
...
"@ -QueryTimeout 5 -TrustServerCertificate -ApplicationName 'xFACts Control Center'
                if ($cpuRows) {
                    $cpuRow = $cpuRows | Select-Object -First 1
