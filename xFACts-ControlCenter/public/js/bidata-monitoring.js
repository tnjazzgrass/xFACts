...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('date-modal-overlay').addEventListener('click', function(e) {
        if (e.target === this) closeDateRangeModal();
    });
});
...
function loadCurrentBuildExecution() {
    engineFetch('/api/bidata/step-progress')
        .then(function(data) {
            if (!data) return;
            if (data.error) { showError('Step progress: ' + data.error); return; }
            clearError();
            renderCurrentBuildExecution(data);
        })
        .catch(function(err) { showError('Failed to load step progress: ' + err.message); });
}
...
function loadDurationTrend(days, fromDate, toDate) {
    ...
    engineFetch(url)
        .then(function(data) {
            if (!data) return;
            if (data.error) { showError('Duration trend: ' + data.error); return; }
            clearError();
            currentTrendData = data;
            renderDurationTrend(data);
        })
        .catch(function(err) { showError('Failed to load duration trend: ' + err.message); });
}
...
function loadBuildHistory() {
    engineFetch('/api/bidata/build-history')
        .then(function(data) {
            if (!data) return;
            if (data.error) { showError('Build history: ' + data.error); return; }
            clearError();
            currentHistoryData = data;
            renderBuildHistory(data);
        })
        .catch(function(err) { showError('Failed to load build history: ' + err.message); });
}
...
function loadBuildDetails(buildId, dateStr) {
    engineFetch('/api/bidata/build-details?build_id=' + buildId)
        .then(function(data) {
            if (!data) return;
            if (data.error) { showError('Build details: ' + data.error); return; }
            renderBuildDetails(data, dateStr);
            if (dateStr) highlightTrendBar(dateStr);
        })
        .catch(function(err) { showError('Failed to load build details: ' + err.message); });
}
...
// Render: Live Activity - Pipeline Card Style
...
function renderLiveActivity(data) {
    ...
    if (build.status === 'IN_PROGRESS') {
        ...
        html += '<div class="activity-card-body"><div class="activity-value">' + formatTime(build.start_dttm) + '</div><div class="activity-detail">' + formatDate(build.build_date) + '</div></div>';
        html += '</div>';
        // Elapsed card
        html += '<div class="activity-card status-running">';
        ...
        if (data.avg_duration_seconds && elapsed > 0) {
            ...
            if (remaining > 0) {
                ...
                html += '<div class="activity-card-body"><div class="activity-value">~' + formatTimeOnly(eta) + '</div><div class="activity-detail">Estimated completion</div></div>';
                html += '</div>';
            }
        }
        ...
        html += '<div class="activity-card status-completed">';
        html += '<div class="activity-card-header"><span class="activity-name">Started</span><span class="activity-icon">🕐</span></div>';
        html += '<div class="activity-card-body"><div class="activity-value">' + formatTime(build.start_dttm) + '</div><div class="activity-detail">Start time</div></div>';
        html += '</div>';
        // Completed card
        html += '<div class="activity-card status-completed">';
        html += '<div class="activity-card-header"><span class="activity-name">Completed</span><span class="activity-icon">🏁</span></div>';
        html += '<div class="activity-card-body"><div class="activity-value">' + formatTime(build.end_dttm) + '</div><div class="activity-detail">End time</div></div>';
        html += '</div>';
        // Duration card
        html += '<div class="activity-card status-completed">';
        ...
        html += '<div class="activity-card-body"><div class="activity-value">' + formatTime(build.end_dttm) + '</div><div class="activity-detail">Failure time</div></div>';
        html += '</div>';
        // Failed Step card
...
// Render: Duration Trend - Full Width Chart with Conditional Date Labels
function renderDurationTrend(data) {
    ...
    var durations = points.map(function(d) { return d.total_execution_seconds > 0 ? d.total_execution_seconds : d.duration_seconds; }).filter(function(d) { return d > 0; });
    if (durations.length === 0) { container.innerHTML = '<div class="no-data">No duration data available</div>'; return; }
    ...
    if (data.stats.min_seconds) html += '<div class="stat"><span class="stat-label">Min</span> <span class="stat-value">' + formatSecondsToHHMMSS(data.stats.min_seconds) + '</span></div>';
    if (data.stats.max_seconds) html += '<div class="stat"><span class="stat-label">Max</span> <span class="stat-value">' + formatSecondsToHHMMSS(data.stats.max_seconds) + '</span></div>';
...
// Render: Build History - with Month Summary Table
...
// Render: Build Details Slideout - for single build
...
// Render: Multiple Builds for Date Slideout
...
function closeDateRangeModal() { document.getElementById('date-modal-overlay').classList.remove('open'); }
...
// Chart Interaction - now loads all builds for date
...
function highlightTrendBar(dateStr) {
    ...
    if (bar) { bar.classList.add('highlighted'); bar.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); }
}

// Tree Toggle - Year level
...
// Tree Toggle - Month level (from table row, toggles sibling details row)
...
// Slideout
function openSlideout() { document.getElementById('build-slideout').classList.add('open'); document.getElementById('slideout-overlay').classList.add('open'); }
function closeSlideout() { document.getElementById('build-slideout').classList.remove('open'); document.getElementById('slideout-overlay').classList.remove('open'); document.querySelectorAll('.trend-bar-container.highlighted').forEach(function(el) { el.classList.remove('highlighted'); }); }

// Utilities
function getStatusClass(status) {
    switch (status) { case 'COMPLETED': return 'status-completed'; case 'FAILED': return 'status-failed'; case 'IN_PROGRESS': return 'status-in-progress'; default: return ''; }
}
function formatDate(dateStr) { if (!dateStr) return 'N/A'; var dt = new Date(dateStr + 'T00:00:00'); return dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }); }
function formatTime(dateTimeStr) { if (!dateTimeStr) return 'N/A'; return new Date(dateTimeStr).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }); }
function formatTimeOnly(date) { return date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }); }
function formatDuration(seconds) { var h = Math.floor(seconds / 3600); var m = Math.floor((seconds % 3600) / 60); var s = Math.floor(seconds % 60); return h + ':' + String(m).padStart(2, '0') + ':' + String(s).padStart(2, '0'); }
function formatSecondsToHHMMSS(seconds) { if (seconds === null || seconds === undefined) return '-'; var h = Math.floor(seconds / 3600); var m = Math.floor((seconds % 3600) / 60); var s = Math.floor(seconds % 60); return h + ':' + String(m).padStart(2, '0') + ':' + String(s).padStart(2, '0'); }
function calculateElapsed(startTimeStr) { if (!startTimeStr) return 0; return Math.floor((new Date() - new Date(startTimeStr)) / 1000); }
function escapeHtml(str) { if (!str) return ''; var div = document.createElement('div'); div.textContent = str; return div.innerHTML; }
function showError(message) { var el = document.getElementById('connection-error'); if (el) { el.textContent = message; el.style.display = 'block'; } }
function clearError() { var el = document.getElementById('connection-error'); if (el) el.style.display = 'none'; }
