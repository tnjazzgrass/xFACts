...
var serverData = [], queueData = {}, activeServer = null, switchingServer = false, pendingConfirm = null;
...
document.addEventListener('DOMContentLoaded', function() {
    loadRefreshInterval(); loadActiveServer(); loadServerStatus(); loadQueueStatus(); connectEngineEvents(); initEngineCardClicks();
});

function loadRefreshInterval() {
    fetch('/api/config/refresh-interval?page=jboss-monitoring').then(function(r){return r.json();}).then(function(d){if(d&&d.interval_seconds)refreshSeconds=d.interval_seconds;startAutoRefresh();}).catch(function(){startAutoRefresh();});
}
function startAutoRefresh() { if(refreshInterval)clearInterval(refreshInterval); refreshInterval=setInterval(function(){loadServerStatus();loadQueueStatus();loadActiveServer();},refreshSeconds*1000); }

function loadActiveServer() { engineFetch('/api/jboss-monitoring/active-server').then(function(d){if(!d||d.Error)return;activeServer=d.active_server;renderUsersBadge();}).catch(function(){}); }

function loadServerStatus() {
    fetch('/api/jboss-monitoring/status').then(function(r){return r.json();}).then(function(data){
        if(data.error){showError(data.error);return;} hideError(); serverData=data.servers||[];
        ...
        renderServers(serverData); updateTimestamp(data.timestamp);
    }).catch(function(err){showError('Failed to load: '+err.message);});
}

function loadQueueStatus() {
    fetch('/api/jboss-monitoring/queue-status').then(function(r){return r.json();}).then(function(data){
        if(data.error)return; queueData={}; var s=data.servers||[];
...
function renderServers(servers) {
    if(!servers||servers.length===0){for(var i=0;i<3;i++){var b=document.getElementById('server-body-'+i);if(b)b.innerHTML='<div class="no-data">No data</div>';}return;}
    for(var i=0;i<servers.length&&i<3;i++)renderServerCard(i,servers[i]);
    renderUsersBadge();
}

function renderServerCard(index, server) {
    var nameEl=document.getElementById('server-name-'+index), roleEl=document.getElementById('server-role-'+index);
    var bodyEl=document.getElementById('server-body-'+index), cardEl=document.getElementById('server-card-'+index);
    ...
    if(server.http_error_message||(server.http_status_code!==200&&server.http_status_code!==null)) cardEl.classList.add('card-critical');

    if(!server.collected_dttm){bodyEl.innerHTML='<div class="no-data">No snapshots collected yet</div>';return;}
    ...
    if(server.api_server_state){
        var jOk=server.api_server_state==='running', jWarn=server.api_server_state==='reload-required';
        var jCls=jOk?'badge-ok':(jWarn?'badge-warn':'badge-fail'), jDot=jOk?'dot-ok':(jWarn?'dot-warn':'dot-fail');
    ...
    if(server.jvm_heap_used_mb!==null){
        ...
        var hCls=hp>=90?'critical':(hp>=75?'warning':'healthy'), hBar=hp>=90?'bar-critical':(hp>=75?'bar-warning':'bar-healthy');
        ...
        if(ex.length>0) h+='<div class="mini-card-secondary">'+ex.join(' · ')+'</div>';
        h+='</div>';
    }
    ...
    if(server.ds_active_count!==null){
        ...
        var dCls=dp>=80?'critical':(dp>=50?'warning':'healthy'), dBar=dp>=80?'bar-critical':(dp>=50?'bar-warning':'bar-healthy');
        h+='<div class="mini-card">';
    ...
    // === MINI-CARD: Transactions (redesigned with per-counter rows + deltas) ===
    ...
    // === MINI-CARD: HTTP Server (with deltas) ===
    if(server.undertow_request_count!==null){
        ...
        h+='<span class="delta-zero">'+(server.undertow_max_proc_ms !== null ? fmtN(server.undertow_max_proc_ms)+' ms' : '\u2014')+'</span>';
    ...
    // === MINI-CARD: JMS Queues (accordion with active/inactive split) ===
    ...
    if(sq){
        ...
        if(queueStats.stuck > 0){
            h+='<div class="alert-row">'+queueStats.stuck+' queue'+(queueStats.stuck!==1?'s':'')+' stuck \u2014 pending messages with no consumers or no delivery</div>';
        } else if(queueStats.totalAdded > 0){
            h+='<div class="queue-summary-ok">'+queueStats.active.length+' queue'+(queueStats.active.length!==1?'s':'')+' active \u00B7 '+fmtN(queueStats.totalAdded)+' messages this cycle</div>';
        } else if(queueStats.processing > 0){
            h+='<div class="queue-summary-ok">'+queueStats.processing+' queue'+(queueStats.processing!==1?'s':'')+' processing \u2014 '+fmtN(queueStats.totalPending)+' in flight</div>';
        }
...
function buildDeltaRow(label, cumulative, delta, severity, pctOf) {
    ...
    if(delta !== null && delta > 0) {
        ...
        h+='<span class="delta-zero">\u2014</span>';
    }

    // Cumulative total (right side, always muted) with "since restart" label
    // Include percentage of total when pctOf is provided and both values are non-null
...
function computeQueueDeltas(serverQueue) {
    ...
    var deltas = {};
    // Start with last-known deltas as fallback
    ...
    // Store baseline — preserve known-good values for missing queues
    var values = {};
...
function classifyQueues(queues, queueDeltas) {
    ...
    for (var i = 0; i < queues.length; i++) {
        ...
        // Classify stuck/processing for pending messages
        ...
        // Track total throughput this cycle
        if (addedDelta !== null && addedDelta > 0) stats.totalAdded += addedDelta;

        // Active = any sign of life this cycle
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if(showDelta) {
            if (delta !== null && delta > 0) {
                ...
                h+='<td class="q-num q-delta-zero">'+(delta !== null ? '0' : '\u2014')+'</td>';
            }
        }
...
var INFO = {
    'overview': {
        ...
        body: '<p>This page monitors the three JBoss application servers. These servers run <strong>JBoss</strong> \u2014 the application server that hosts Debt Manager. Every time you open DM, run a job, post a payment, or load a batch, JBoss is the engine doing the work.</p>' +
            '<p>Think of each server card as a dashboard for one engine:</p>' +
            '<div class="info-list">' +
                '<div class="info-item"><strong>Status badges</strong> (top row) are the quick health check \u2014 is the front door open, is the engine running, is JBoss responding?</div>' +
                '<div class="info-item"><strong>JVM Heap</strong> is how much memory the engine is using. Like a car\u2019s fuel gauge, but for RAM.</div>' +
                ...
                '<div class="info-item"><strong>HTTP Server</strong> is the web server layer inside JBoss that handles all incoming requests \u2014 page loads, API calls, everything.</div>' +
            ...
            '</div>' +
            '<p>The key insight: <strong>these queues are independent per server, not shared.</strong> When a server freezes, all its queued work stops. The other servers can\u2019t pick it up. That\u2019s why we monitor each server separately.</p>'
    },
    'status-badges': {
        title: 'Status Badges',
        body: '<p>The status badges across the top of each card are the fastest health check:</p>' +
            '<div class="info-list">' +
                '<div class="info-item"><strong>HTTP</strong> \u2014 Can we reach the server? Every 60 seconds, xFACts hits the DM splash page. If it responds with HTTP 200, the server is alive. The number in parentheses is the response time in milliseconds. Under 50ms is fast; over 500ms is sluggish. If this badge goes red, the server is unreachable \u2014 likely frozen or down.</div>' +
                '<div class="info-item"><strong>Service</strong> \u2014 Is the Windows service running? This checks the actual DebtManager-Host service on the server. It can show Running even when the application is frozen (which is the whole problem \u2014 the process is alive but not responding).</div>' +
                '<div class="info-item"><strong>JBoss</strong> \u2014 Is the application server in a healthy state? Values: <em>running</em> (good), <em>reload-required</em> (config change needs a restart), <em>stopped</em> (down). This comes from the JBoss Management API and only appears when API access is available.</div>' +
                '<div class="info-item"><strong>Uptime</strong> \u2014 How long since the last OS reboot. If one server shows significantly less uptime than the others, it was recently restarted. Useful for context when looking at metrics that reset on restart.</div>' +
            '</div>'
    },
    'jvm-heap': {
        title: 'JVM Heap Memory',
        body: '<p>JBoss runs inside a Java Virtual Machine (JVM), and the <strong>heap</strong> is the chunk of memory Java uses to store everything the application is working with \u2014 user sessions, cached data, objects being processed, queued work items.</p>' +
            ...
            '<div class="info-thresholds">' +
                '<span><span class="info-green">\u25CF Under 75%</span> \u2014 Normal. Java\u2019s garbage collector has room to work.</span>' +
                '<span><span class="info-yellow">\u25CF 75% \u2013 90%</span> \u2014 Elevated. Garbage collection runs more frequently, which can slow things down. Worth watching but not necessarily a problem.</span>' +
                '<span><span class="info-red">\u25CF Over 90%</span> \u2014 Critical. Java is spending more time cleaning up memory than doing useful work. Performance degrades significantly. If it hits 100%, the application may freeze entirely.</span>' +
            '</div>' +
            ...
            '<p><strong>OS (Working Set)</strong> is the total memory the JBoss process uses from the operating system\u2019s perspective. This is always larger than the heap because it includes non-heap, native memory, and OS overhead.</p>'
    },
    'jvm-threads': {
        title: 'JVM Threads',
        body: '<p>A <strong>thread</strong> is a unit of work inside JBoss. Every user request, every background job, every internal process runs on a thread. More threads = more things happening simultaneously.</p>' +
            ...
            '<div class="info-list">' +
                '<div class="info-item"><strong>Stable thread count (~150\u2013300)</strong> \u2014 Normal. JBoss reuses threads from a pool.</div>' +
                '<div class="info-item"><strong>Climbing thread count</strong> \u2014 Potential concern. Could indicate requests piling up (threads waiting on slow operations) or a thread leak.</div>' +
                '<div class="info-item"><strong>Thread count near peak</strong> \u2014 The server is as busy as it\u2019s ever been. Not necessarily bad, but worth correlating with other metrics.</div>' +
            '</div>'
    },
    'db-pool': {
        ...
        body: '<p>JBoss keeps a <strong>pool</strong> of pre-opened database connections ready to use. When application code needs to query the database, it borrows a connection from the pool, uses it, and returns it. This is much faster than opening a new connection every time.</p>' +
            ...
            '<div class="info-list">' +
                '<div class="info-item"><strong>Idle</strong> \u2014 Connections sitting in the pool waiting to be used. More idle = more headroom.</div>' +
                '<div class="info-item"><strong>Peak</strong> \u2014 Highest concurrent in-use count since restart. The historical high-water mark.</div>' +
                '<div class="info-item"><strong>Avg get time</strong> \u2014 Average time to borrow a connection. Under 5ms is normal. Rising times mean contention.</div>' +
            '</div>' +
            '<div class="info-thresholds">' +
                '<span><span class="info-green">\u25CF Under 50% utilization</span> \u2014 Plenty of room.</span>' +
                '<span><span class="info-yellow">\u25CF 50% \u2013 80%</span> \u2014 Getting busy. Watch the trend.</span>' +
                '<span><span class="info-red">\u25CF Over 80%</span> \u2014 Pool is under pressure. If it hits 100%, threads start waiting and the application slows down.</span>' +
            '</div>' +
    ...
    'transactions': {
        title: 'Transactions',
        body: '<p>A <strong>transaction</strong> is a unit of database work \u2014 a query, an update, a batch of operations that either all succeed or all fail. Every time DM saves data, posts a payment, or processes a batch, it\u2019s wrapped in a transaction.</p>' +
            ...
            '<div class="info-list">' +
                '<div class="info-item"><strong>In-flight</strong> \u2014 Transactions currently in progress. A few at a time is normal. A sustained high number means transactions are taking longer than usual.</div>' +
                '<div class="info-item"><strong>Committed</strong> \u2014 Successfully completed transactions. This is the throughput indicator. Higher during business hours, lower at night. <span class="info-green">Informational, not concerning.</span></div>' +
                '<div class="info-item"><strong>Timed out</strong> \u2014 Transactions that took longer than the 15-minute timeout. <span class="info-red">Any increase means something was stuck for a long time.</span> This is rare and always worth investigating.</div>' +
                '<div class="info-item"><strong>Rollbacks</strong> \u2014 Transactions that were intentionally reversed by the application. <span class="info-yellow">Some rollbacks are normal</span> (validation failures, duplicate checks). A sudden spike may indicate a problem.</div>' +
                '<div class="info-item"><strong>Heuristics</strong> \u2014 Rare edge cases where the transaction manager had to make a unilateral decision about a two-phase commit. Typically stays at zero or a small constant. Not usually concerning unless it\u2019s actively growing.</div>' +
            '</div>'
    },
    'http-server': {
        title: 'HTTP Server (Undertow)',
        body: '<p><strong>Undertow</strong> is the web server built into JBoss. It handles every HTTP request that comes into the DM application \u2014 page loads, API calls, file downloads, everything. Think of it as the front desk: all traffic passes through here.</p>' +
            ...
            '<div class="info-list">' +
                '<div class="info-item"><strong>Requests</strong> \u2014 Total HTTP requests handled. The "this cycle" number shows how busy the server is right now. Higher during business hours, lower at night. This is informational, not concerning by itself.</div>' +
                '<div class="info-item"><strong>Errors</strong> \u2014 HTTP requests that returned an error. <span class="info-green">Zero is the goal.</span> <span class="info-red">Any increase here needs attention</span> \u2014 it could mean application errors, misconfigured endpoints, or server-side failures.</div>' +
                '<div class="info-item"><strong>Data sent</strong> \u2014 Bytes served this cycle and total since restart. A spike in data with stable request count could mean large reports or file exports are being pulled.</div>' +
                '<div class="info-item"><strong>Processing</strong> \u2014 Total server-side processing time spent handling requests. Combined with request count, gives a sense of how hard the server is working. Requires <code>record-request-start-time</code> to be enabled on the Undertow HTTP listener (pending next JBoss restart).</div>' +
                '<div class="info-item"><strong>Max request</strong> \u2014 The single slowest HTTP request the server has handled since JBoss started. A high-water mark that resets on restart. Useful for spotting outlier requests. Also requires <code>record-request-start-time</code>.</div>' +
            '</div>' +
            '<p>If you see <span class="info-red">"IO queue: N waiting"</span> at the bottom, that means incoming requests are piling up faster than Undertow can process them. The server is overwhelmed \u2014 this typically only happens during a freeze or extreme load.</p>'
    },
    'jms-queues': {
        title: 'JMS Queues',
        body: '<p><strong>JMS (Java Message Service) queues</strong> are how JBoss organizes internal work. When you trigger an action in DM \u2014 release a batch, start a payment import, generate a document \u2014 it doesn\u2019t happen immediately in your browser session. Instead, a <em>message</em> gets placed in a queue, and a background <em>consumer</em> picks it up and processes it.</p>' +
            '<p>This is the same concept as a print queue: you click "print" and the job goes into a queue where the printer processes it when it\u2019s ready.</p>' +
            '<p><strong>Key concepts:</strong></p>' +
            '<div class="info-list">' +
                '<div class="info-item"><strong>Pending</strong> \u2014 Messages waiting to be picked up. Zero is healthy. A sustained non-zero means work is backing up \u2014 consumers aren\u2019t keeping pace.</div>' +
                '<div class="info-item"><strong>Delivering</strong> \u2014 Messages picked up by a consumer but not yet finished. These are in-flight. High delivering with stable pending = normal processing. High delivering with rising pending = consumers are stuck.</div>' +
                '<div class="info-item"><strong>Consumers</strong> \u2014 The number of background workers listening on this queue. Zero consumers on an active queue means nobody is processing work. The requestQueue typically has ~70 consumers; most others have 1\u20136.</div>' +
                '<div class="info-item"><strong>Added</strong> \u2014 Total messages ever added to this queue since JBoss started. The difference between snapshots shows throughput.</div>' +
            '</div>' +
            '<p><strong>Critical fact: Queues are independent per server.</strong> They are not shared across the three DM app servers. When APP2 freezes, all work queued on APP2 stops. APP and APP3 cannot pick it up. Each server\u2019s queues are an isolated world.</p>' +
...
// === USERS BADGE ===
function renderUsersBadge(){var e=document.querySelector('.users-badge');if(e)e.remove();if(!activeServer||!serverData||serverData.length===0)return;for(var i=0;i<serverData.length;i++){if(serverData[i].server_name.toUpperCase()===activeServer.toUpperCase()){var hdr=document.querySelector('#server-card-'+i+' .server-card-header');if(!hdr)return;var b=document.createElement('span');b.className='users-badge';b.textContent='Users';b.title='SharePoint Debt Manager link points here';if(window.isAdmin){b.classList.add('clickable');b.title='Click to change SharePoint link target';b.onclick=function(ev){ev.stopPropagation();openSwitchModal();};}hdr.appendChild(b);return;}}}

// === SERVER SWITCH MODAL ===
function openSwitchModal(){if(!window.isAdmin)return;document.getElementById('dm-modal-overlay').classList.add('visible');document.getElementById('dm-modal').classList.add('visible');if(!switchingServer){document.getElementById('dm-status').textContent='';document.getElementById('dm-status').className='dm-status';}renderModalButtons();}
function closeSwitchModal(){if(switchingServer)return;document.getElementById('dm-modal-overlay').classList.remove('visible');document.getElementById('dm-modal').classList.remove('visible');}
function renderModalButtons(){document.querySelectorAll('.dm-server-btn').forEach(function(btn){var s=btn.getAttribute('data-server');if(s.toUpperCase()===(activeServer||'').toUpperCase()){btn.classList.add('active');btn.disabled=true;}else{btn.classList.remove('active');btn.disabled=false;}});}
function selectServer(server){if(server.toUpperCase()===(activeServer||'').toUpperCase())return;var from=activeServer?activeServer.replace('DM-PROD-',''):'?';var to=server.replace('DM-PROD-','');closeSwitchModal();showConfirm('Switch DM App Server','Redirect SharePoint link from '+from+' to '+to+'?\n\nThis immediately affects all users.','Switch to '+to,'danger',function(){openSwitchModal();doSwitchServer(server);});}
function doSwitchServer(server){switchingServer=true;document.getElementById('dm-modal-overlay').classList.add('locked');document.getElementById('dm-modal').classList.add('locked');document.querySelectorAll('.dm-server-btn').forEach(function(b){b.disabled=true;});setModalStatus('Switching to '+server.replace('DM-PROD-','')+'... May take up to 90 seconds.','working');engineFetch('/api/jboss-monitoring/switch-server',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target_server:server})}).then(function(d){switchingServer=false;document.getElementById('dm-modal-overlay').classList.remove('locked');document.getElementById('dm-modal').classList.remove('locked');if(!d){renderModalButtons();return;}if(d.Error){setModalStatus(d.Error,'error');renderModalButtons();return;}activeServer=d.active_server;renderModalButtons();renderUsersBadge();setModalStatus('Switched to '+d.active_server.replace('DM-PROD-','')+' by '+d.performed_by,'success');}).catch(function(e){switchingServer=false;document.getElementById('dm-modal-overlay').classList.remove('locked');document.getElementById('dm-modal').classList.remove('locked');setModalStatus(e.message,'error');renderModalButtons();});}
function setModalStatus(msg,type){var el=document.getElementById('dm-status');el.textContent=msg;el.className='dm-status '+(type||'');}

// === CONFIRM DIALOG ===
function showConfirm(title,message,btnText,type,fn){document.getElementById('confirm-title').textContent=title;document.getElementById('confirm-message').textContent=message;var b=document.getElementById('confirm-btn');b.textContent=btnText;b.className='confirm-btn action '+type;pendingConfirm=fn;document.getElementById('confirm-overlay').classList.add('visible');}
...
function fmtC(n){if(n===null||n===undefined)return'-';if(n>=1000000)return(n/1000000).toFixed(1)+'M';if(n>=10000)return(n/1000).toFixed(0)+'K';if(n>=1000)return(n/1000).toFixed(1)+'K';return n.toLocaleString();}
function formatBytes(b){if(b===null||b===undefined)return'-';if(b>=1073741824)return(b/1073741824).toFixed(1)+' GB';if(b>=1048576)return(b/1048576).toFixed(1)+' MB';if(b>=1024)return(b/1024).toFixed(0)+' KB';return b+' B';}
function formatUptime(hours){if(hours===null||hours===undefined)return'-';hours=Number(hours);if(isNaN(hours))return'-';var d=Math.floor(hours/24),r=Math.floor(hours%24);return d>0?d+'d '+r+'h':r+'h';}
function escapeHtml(s){if(!s)return'';var d=document.createElement('div');d.textContent=s;return d.innerHTML;}
function updateTimestamp(ts){var el=document.getElementById('last-update');if(el&&ts)el.textContent=ts;}
function showError(msg){var el=document.getElementById('connection-error');if(el){el.textContent=msg;el.classList.add('visible');}}
function hideError(){var el=document.getElementById('connection-error');if(el)el.classList.remove('visible');}
...
function pageRefresh(){var btn=document.querySelector('.page-refresh-btn');if(btn){btn.classList.add('spinning');setTimeout(function(){btn.classList.remove('spinning');},800);}refreshAll();}
