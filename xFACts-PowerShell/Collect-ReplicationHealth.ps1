...
.DESCRIPTION
    ...
    Collection layers: registry sync discovers publications and subscribers
    from the distribution DB; agent health, queue depth, and throughput are
    captured as per-agent snapshots each cycle; event detection compares
    against the previous snapshot for state changes; tracer tokens measure
    end-to-end latency on a configurable interval.

    Source data comes from the distribution database agent catalog and history
    tables (MSdistribution_agents, MSdistribution_history, MSlogreader_agents,
    MSlogreader_history), the registered subscriber names (MSsubscriber_info),
    publication metadata (MSpublications), queue depth
    (sp_replmonitorsubscriptionpendingcmds), and the publisher tracer-token
    procedures (sp_posttracertoken, sp_helptracertokenhistory).

    All queries return numeric or short-string data (agent metadata, run
    status, queue depths, latency values), so the shared SQL helpers are
    called without MaxCharLength. XML stats blocks in agent_message are
    replaced with a summary string because that data is redundant with the
    structured throughput columns on the same row.

    Deployed to E:\xFACts-PowerShell on FA-SQLDBB. The service account running
    this script must have SQL access to DM-PROD-DB (distribution database) and
    to the publisher database (crs5_oltp). xFACts-OrchestratorFunctions.ps1
...
.PARAMETER Execute
    Perform writes. Without this flag, the script exits immediately. No preview
    mode - this is a high-frequency collector with no dry-run path.
...
.NOTES
    File Name : Collect-ReplicationHealth.ps1
    Location  : E:\xFACts-PowerShell\Collect-ReplicationHealth.ps1
...
#             the rpm_ prefix to all helper functions, renamed the Safe-Sql*
#             helpers to ConvertTo-rpm_Sql* (approved verb), converted inline
#             dividers to sub-section markers, and stripped trailing whitespace.
...
# 2026-02-18  Initial implementation. Registry discovery from distribution
#             database; agent health, queue depth, and throughput collection;
#             Log Reader agent collection; event detection via state-change
#             comparison; tracer token posting and result collection; BIDATA
#             build correlation for event tagging; Orchestrator v2
#             FIRE_AND_FORGET integration.

<# ============================================================================
   PARAMETERS: SCRIPT PARAMETERS
   ----------------------------------------------------------------------------
   Script-level parameters: target instances and databases, the execute
   switch, the force switch, and the orchestrator callback identifiers.
   Prefix: (none)
   ============================================================================ #>
...
<# ============================================================================
   ...
   Initialize-XFActsScript, the shared SQL helpers, Write-Log, and the
...
<# ============================================================================
   INITIALIZATION: SCRIPT INITIALIZATION
   ----------------------------------------------------------------------------
   One-time setup of shared infrastructure and application identity, plus
   the hard execute guard. This collector has no preview mode: without
   -Execute it exits immediately.
   Prefix: (none)
   ============================================================================ #>
...
<# ============================================================================
   FUNCTIONS: COLLECTION HELPERS
   ----------------------------------------------------------------------------
   Configuration lookup, run-status decoding, and SQL-literal formatting
   helpers used throughout the collection body.
   Prefix: rpm
   ============================================================================ #>

# Look up a ServerOps.Replication GlobalConfig setting value by name.
function Get-rpm_ConfigValue {
...
# Map a replication agent run-status integer to its display description.
function Get-rpm_RunStatusDesc {
    param([int]$RunStatus)
...
# Format a value as a SQL string literal: NULL for null/DBNull, otherwise
# single-quote-escaped and optionally truncated to MaxLength.
function ConvertTo-rpm_SqlString {
...
# Format a value as a SQL int literal: NULL for null/DBNull, else cast to int.
function ConvertTo-rpm_SqlInt {
    param($Value)
...
# Format a value as a SQL float literal: NULL for null/DBNull, else cast to float.
function ConvertTo-rpm_SqlFloat {
    param($Value)
...
# Format a value as a SQL smallint literal: NULL for null/DBNull, else cast to int.
function ConvertTo-rpm_SqlSmallInt {
    param($Value)
...
# Format a value as a SQL datetime literal: NULL for null/DBNull, else a
# quoted yyyy-MM-dd HH:mm:ss.fff string.
function ConvertTo-rpm_SqlDateTime {
    param($Value)
...
<# ============================================================================
   EXECUTION: SCRIPT EXECUTION
   ----------------------------------------------------------------------------
   The procedural collection body: registry discovery and sync, active-
   registry load, agent health and throughput snapshots, inline event
   detection, and conditional tracer-token latency measurement, followed
   by the orchestrator completion callback.
   Prefix: (none)
   ============================================================================ #>
...
SET agent_id = $(ConvertTo-rpm_SqlInt $agent.agent_id),
    agent_name = $(ConvertTo-rpm_SqlString $agent.agent_name 100),
    publisher_id = $(ConvertTo-rpm_SqlSmallInt $agent.publisher_id),
    publication_id = $(ConvertTo-rpm_SqlInt $agent.publication_id),
    publication_type = $(ConvertTo-rpm_SqlInt $agent.publication_type),
    publication_type_desc = $(ConvertTo-rpm_SqlString $agent.publication_type_desc 20),
    subscriber_id = $(ConvertTo-rpm_SqlSmallInt $agent.subscriber_id),
    subscription_type = $(ConvertTo-rpm_SqlInt $agent.subscription_type),
    subscription_type_desc = $(ConvertTo-rpm_SqlString $agent.subscription_type_desc 10),
    is_dropped = 0,
...
INSERT INTO ServerOps.Replication_PublicationRegistry (
    ...
    $(ConvertTo-rpm_SqlSmallInt $agent.publisher_id),
    $(ConvertTo-rpm_SqlString $agent.publisher_db 128),
    $(ConvertTo-rpm_SqlInt $agent.publication_id),
    $(ConvertTo-rpm_SqlString $agent.publication 128),
    $(ConvertTo-rpm_SqlInt $agent.publication_type),
    $(ConvertTo-rpm_SqlString $agent.publication_type_desc 20),
    $(ConvertTo-rpm_SqlSmallInt $agent.subscriber_id),
    $(ConvertTo-rpm_SqlString $subscriberName 128),
    $(ConvertTo-rpm_SqlString $agent.subscriber_db 128),
    $(ConvertTo-rpm_SqlInt $agent.subscription_type),
    $(ConvertTo-rpm_SqlString $agent.subscription_type_desc 10),
    $(ConvertTo-rpm_SqlString $agent.agent_name 100),
    $(ConvertTo-rpm_SqlInt $agent.agent_id),
    'Distribution'
)
"@
...
if ($null -ne $logReaderAgents) {
    ...
    foreach ($lr in $lrList) {
        ...
        if ($registryLookup.ContainsKey($key)) {
            ...
            $updateQuery = @"
UPDATE ServerOps.Replication_PublicationRegistry
SET agent_id = $(ConvertTo-rpm_SqlInt $lr.agent_id),
    agent_name = $(ConvertTo-rpm_SqlString $lr.agent_name 100),
    is_dropped = 0,
    dropped_detected_dttm = NULL,
    modified_dttm = GETDATE()
WHERE publication_registry_id = $regId
...
INSERT INTO ServerOps.Replication_PublicationRegistry (
    ...
    $(ConvertTo-rpm_SqlString $lr.publisher_db 128),
    $(ConvertTo-rpm_SqlString $lr.publisher_db 128),
    $(ConvertTo-rpm_SqlString $lr.publisher_db 128),
    $(ConvertTo-rpm_SqlString $lr.publisher_db 128),
    0, 'Push',
    $(ConvertTo-rpm_SqlString $lr.agent_name 100),
    $(ConvertTo-rpm_SqlInt $lr.agent_id),
    'LogReader'
)
"@
            Invoke-SqlNonQuery -Query $insertQuery | Out-Null
...
"@
        ...
        if ($null -ne $pending) {
            $pendingCmdCount = ConvertTo-rpm_SqlInt $pending.pendingcmdcount
            $estimatedProcessingSeconds = ConvertTo-rpm_SqlInt $pending.estimatedprocesstime
        }
    }
...
INSERT INTO ServerOps.Replication_AgentHistory (
    ...
    $regId,
    $(ConvertTo-rpm_SqlInt $health.runstatus),
    $(ConvertTo-rpm_SqlString $agentMessage),
    $(ConvertTo-rpm_SqlDateTime $health.time),
    $(ConvertTo-rpm_SqlInt $health.error_id),
    $pendingCmdCount,
    $estimatedProcessingSeconds,
    $(ConvertTo-rpm_SqlFloat $health.delivery_rate),
    $(ConvertTo-rpm_SqlInt $health.delivered_transactions),
    $(ConvertTo-rpm_SqlInt $health.delivered_commands),
    $(ConvertTo-rpm_SqlInt $health.average_commands),
    $(ConvertTo-rpm_SqlInt $health.delivery_latency),
    '$collectedDttm'
)
"@
    ...
    $currentRunStatusDesc = Get-rpm_RunStatusDesc $currentRunStatus
    ...
    if ($null -ne $previousRunStatus -and $currentRunStatus -ne $previousRunStatus) {
        $previousRunStatusDesc = Get-rpm_RunStatusDesc $previousRunStatus
...
"@
        ...
        $errorDetail = "NULL"
        $errorId = ConvertTo-rpm_SqlInt $health.error_id
...
"@
            ...
            if ($null -ne $errorResult -and $null -ne $errorResult.error_text -and $errorResult.error_text -isnot [DBNull]) {
                $errorDetail = ConvertTo-rpm_SqlString ($errorResult.error_text.ToString()) 1000
            }
        }
...
$tracerIntervalMinutes = [int](Get-rpm_ConfigValue 'replication_tracer_interval_minutes')
if ($null -eq $tracerIntervalMinutes -or $tracerIntervalMinutes -le 0) { $tracerIntervalMinutes = 5 }

$tracerWaitSeconds = [int](Get-rpm_ConfigValue 'replication_tracer_wait_seconds')
...
INSERT INTO ServerOps.Replication_LatencyHistory (
    ...
    $tokenId,
    $(ConvertTo-rpm_SqlDateTime $th.publisher_commit),
    $(ConvertTo-rpm_SqlDateTime $th.distributor_commit),
    $(ConvertTo-rpm_SqlDateTime $th.subscriber_commit),
    $(ConvertTo-rpm_SqlInt $th.distributor_latency),
    $(ConvertTo-rpm_SqlInt $th.subscriber_latency),
    $(ConvertTo-rpm_SqlInt $th.overall_latency),
    '$collectedDttm'
)
"@
