...
.DESCRIPTION
    Monitors Debt Manager job flows: tracks execution progress, detects stalls,
    validates completions, and alerts on issues. Reads crs5_oltp from a configurable
    AG replica and writes all JobFlow.* updates through the AG listener, detecting
    current PRIMARY/SECONDARY roles automatically and supporting a preview mode.
    Integrated ConfigSync synchronizes JobFlow.FlowConfig with Debt Manager flow
    definitions every cycle, detecting new, deactivated, and reactivated flows.
...
<# ============================================================================
   ...
   in dbo.System_Metadata (component JobFlow); this section records what changed.
   Prefix: (none)
   ============================================================================ #>

# 2026-06-19  Conformed the file to the xFACts PowerShell file format spec: block-comment
#             section banners in canonical order, comment-based-help file header with
#             .COMPONENT, dedicated CHANGELOG section, jfm-prefixed local functions, and
#             single-line purpose comments. Deleted the local Get-AGReplicaRoles copy and
#             switched AG detection to the shared Get-AGReplicaRoles -AGName. Promoted the
#             local Get-SourceData to xFACts-OrchestratorFunctions.ps1 (shared, explicit
#             -ReadServer/-SourceDB) and call sites now pass those parameters. Renamed
#             Initialize-Configuration to Initialize-jfm_MonitorConfig. Converted Write-Host
#             output to the shared Write-Console family. Fixed the Stall and MissingFlow
#             Jira ticket calls that referenced an undefined $executionDate/$triggerValue,
#             which queued blank @DueDate/@TriggerValue; both now use $currentDate.
# 2026-04-28  Standardized Teams alerting via Send-TeamsAlert shared function. Converted both
#             Teams alert sites from EXEC sp_QueueAlert to Send-TeamsAlert (Stall in Step 5,
#             MissingFlow in Step 6). Inline Teams.RequestLog dedup queries removed (handled
#             by the shared function). Added title icons for visual scanning consistency.
#             Severity, color, and trigger_value schemes unchanged. Jira queue calls left
#             untouched - Jira standardization is a separate audit.
# 2026-03-15  Stall detection IDLE path fix. Moved counterBefore read before IDLE exit checks
#             in Step 5 so RESET events log correctly when the system goes from stalled to
#             idle. Step 2 dedup optimization: date-filtered query replaces full table scan.
#             Removed dead exclusionClause code block from Step 2 and debug Write-Log lines
#             from Step 6.
# 2026-03-14  Validation date filter fix. Removed the execution_date >= today filter from the
#             Step 6 query, which permanently orphaned flows completing after midnight in
#             COMPLETE state and blocked early exit every cycle. Added tracking_id > 0 to the
#             early-exit unresolved-flows check so the backfill row (tracking_id 0) is not
#             counted. Reset Status rows for skipped steps on early exit so CC cards do not
#             show stale values from the last full pipeline run.
...
# 2026-03-03  Early exit now checks xFACts for unresolved flows, fixing the last flow in a run
#             staying in EXECUTING. Early exit when no active job activity is detected skips
#             Steps 1-6 for performance. Fixed incorrect TriggerType on Jira queue calls in
#             the stall and missing-flow alert paths.
# 2026-02-12  Bug fix: missing RESET event in StallDetectionLog on IDLE transition after a
#             stall counter increment.
# 2026-02-11  Bug fix: stall alert never firing. StallDetectionLog INSERT moved after the
#             dedup check.
# 2026-02-04  Orchestrator v2 integration: TaskId/ProcessId parameters, FIRE_AND_FORGET
#             callback. Relocated to E:\xFACts-PowerShell on FA-SQLDBB.
# 2026-02-01  Integrated ConfigSync into the monitor cycle. Detects new/deactivated/
#             reactivated DM flows, inserts UNCONFIGURED stub rows, and queues Jira tickets
#             for drift. Promoted SourceReplica from JobFlow to the Core GlobalConfig module.
# 2026-01-31  Bug fix: fast-completing flows not tracked. Flow detection now includes
#             COMPLETED status (stts_cd 3).
# 2026-01-29  Initial implementation. AG-aware replica detection, embedded SQL logic,
#             multi-server connectivity, and preview mode support. Replaces
#             JobFlow.sp_StateMonitor.
...
<# ============================================================================
   ...
   SQL access, AG replica resolution, source-data access, alerting, and the callback.
   Prefix: (none)
   ============================================================================ #>
...
<# ============================================================================
   VARIABLES: SERVER AND CONFIG STATE
   ----------------------------------------------------------------------------
   Script-scope state populated by Initialize-jfm_MonitorConfig and read across the
   monitoring steps: resolved AG replica names, read/write targets, the loaded
   GlobalConfig hashtable, and the captured execution date.
   Prefix: jfm
   ============================================================================ #>
...
# Loaded GlobalConfig settings (AGName, SourceReplica, StallThreshold).
$script:Config = @{}
# Execution date captured at start for consistency across all steps.
$script:ExecutionDate = $null

<# ============================================================================
   FUNCTIONS: CONFIGURATION
   ----------------------------------------------------------------------------
   Loads JobFlow monitor configuration from GlobalConfig and resolves the read and
   write SQL Server targets, using shared AG replica detection or a forced override.
   Prefix: jfm
   ============================================================================ #>

# Loads JobFlow monitor GlobalConfig and resolves the read/write SQL servers.
function Initialize-jfm_MonitorConfig {
    param()
...
<# ============================================================================
   FUNCTIONS: MONITORING STEPS
   ----------------------------------------------------------------------------
   The monitor pipeline steps: config sync, flow detection, job capture, progress
   updates, state transitions, stall detection, completion validation, and missing-
   flow detection. Each returns a result hashtable and updates JobFlow.Status.
   Prefix: jfm
   ============================================================================ #>

# Synchronizes JobFlow.FlowConfig with Debt Manager flow definitions and queues drift tickets.
function Step-jfm_ConfigSync {
...
"@
    ...
    $dmFlows = Get-SourceData -Query $dmFlowsQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
# Detects flows started in the last 24 hours and inserts tracking records for new ones.
function Step-jfm_DetectFlows {
    param([bool]$PreviewOnly = $true)
...
"@
        ...
        $newFlows = Get-SourceData -Query $newFlowsQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
"@
                        $expectedJobs = Get-SourceData -Query $expectedJobsQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
"@
                        $jobList = Get-SourceData -Query $jobListQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
# Captures execution detail rows for recently completed jobs into JobFlow.JobExecutionLog.
function Step-jfm_CaptureCompletedJobs {
    param([bool]$PreviewOnly = $true)
...
"@
        ...
        $completedJobs = Get-SourceData -Query $completedJobsQuery -ReadServer $script:ReadServer -SourceDB $SourceDB -Timeout 120
...
# Updates job counts and progress metrics for active flows from the source database.
function Step-jfm_UpdateFlowProgress {
    param([bool]$PreviewOnly = $true)
...
"@
                ...
                $counts = Get-SourceData -Query $countsQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
# Advances flow execution state DETECTED -> EXECUTING -> COMPLETE based on job counts.
function Step-jfm_TransitionFlowStates {
    param([bool]$PreviewOnly = $true)
...
# Detects system-wide stalls by comparing job snapshots between polls and alerts at threshold.
function Step-jfm_DetectStalls {
    param([bool]$PreviewOnly = $true)
...
"@
        ...
        $activeJobs = Get-SourceData -Query $sourceQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
# Validates completed flows against expected jobs and queues tickets for failures.
function Step-jfm_ValidateCompletedFlows {
    param([bool]$PreviewOnly = $true)
            ...
            foreach ($job in @($jobAnalysis)) {
                ...
                if ($validationResult -eq 'TOTAL_FAILURE') {
                    ...
                    $errorTypeQuery = @"
                        ...
                        ORDER BY COUNT(*) DESC
"@
                    $errorTypeResult = Get-SourceData -Query $errorTypeQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
# Detects scheduled flows that have not started by their deadline and alerts.
function Step-jfm_DetectMissingFlows {
    param([bool]$PreviewOnly = $true)
...
<# ============================================================================
   EXECUTION: SCRIPT EXECUTION
   ----------------------------------------------------------------------------
   The monitor run: initialize configuration, run ConfigSync, then either early-exit
   after the missing-flow check when idle or run the full detection/validation
   pipeline, print the summary, and fire the orchestrator completion callback.
   Prefix: (none)
   ============================================================================ #>
...
Write-Console
Write-ConsoleBanner -Label "xFACts JobFlow Monitor" -Color Cyan
...
if (-not (Initialize-jfm_MonitorConfig)) {
...
$stepResults.ConfigSync = Step-jfm_ConfigSync -PreviewOnly $previewOnly
...
"@
$activeJobs = Get-SourceData -Query $activeJobsQuery -ReadServer $script:ReadServer -SourceDB $SourceDB
...
if (-not $dmActive -and -not $xfactsUnresolved) {
    ...
    $stepResults.MissingFlows = Step-jfm_DetectMissingFlows -PreviewOnly $previewOnly
    $earlyExit = $true
...
"@ | Out-Null
    ...
    $stepResults.DetectFlows = Step-jfm_DetectFlows -PreviewOnly $previewOnly

    # Step 2: Capture completed jobs
    $stepResults.CaptureJobs = Step-jfm_CaptureCompletedJobs -PreviewOnly $previewOnly

    # Step 3: Update flow progress
    $stepResults.UpdateProgress = Step-jfm_UpdateFlowProgress -PreviewOnly $previewOnly

    # Step 4: Transition flow states
    $stepResults.TransitionStates = Step-jfm_TransitionFlowStates -PreviewOnly $previewOnly

    # Step 5: Detect stalls
    $stepResults.DetectStalls = Step-jfm_DetectStalls -PreviewOnly $previewOnly

    # Step 6: Validate completed flows
    $stepResults.ValidateFlows = Step-jfm_ValidateCompletedFlows -PreviewOnly $previewOnly
...
if ($earlyExit) {
    Write-Console "  Mode: EARLY EXIT (no active job activity)"
    Write-Console "  Results:"
    Write-Console "    Config Sync:        DM=$($stepResults.ConfigSync.DMFlowCount) | New=$($stepResults.ConfigSync.NewFlows) Deact=$($stepResults.ConfigSync.DeactivatedFlows) React=$($stepResults.ConfigSync.ReactivatedFlows)"
    Write-Console "    Missing Flows:      $($stepResults.MissingFlows.MissingFlows) (Alerts: $($stepResults.MissingFlows.AlertsQueued))"
}
else {
    Write-Console "  Results:"
    Write-Console "    Config Sync:        DM=$($stepResults.ConfigSync.DMFlowCount) | New=$($stepResults.ConfigSync.NewFlows) Deact=$($stepResults.ConfigSync.DeactivatedFlows) React=$($stepResults.ConfigSync.ReactivatedFlows)"
    Write-Console "    Flows Detected:     $($stepResults.DetectFlows.FlowsDetected)"
    Write-Console "    Jobs Captured:      $($stepResults.CaptureJobs.JobsCaptured)"
    Write-Console "    Flows Updated:      $($stepResults.UpdateProgress.FlowsUpdated)"
    Write-Console "    State Transitions:  $($stepResults.TransitionStates.DetectedToExecuting + $stepResults.TransitionStates.ExecutingToComplete)"
    Write-Console "    Stall State:        $($stepResults.DetectStalls.StallState) (Counter: $($stepResults.DetectStalls.Counter))"
    Write-Console "    Flows Validated:    $($stepResults.ValidateFlows.FlowsValidated) (Issues: $($stepResults.ValidateFlows.IssuesFound))"
    Write-Console "    Missing Flows:      $($stepResults.MissingFlows.MissingFlows) (Alerts: $($stepResults.MissingFlows.AlertsQueued))"
}

Write-Console
Write-Console "  Duration: $([int]$scriptDuration.TotalMilliseconds) ms"
Write-Console
...
Write-ConsoleBanner -Label "JobFlow Monitor Complete" -Color Cyan
