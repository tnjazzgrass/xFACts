<#
.SYNOPSIS
    Scans index fragmentation and populates the ServerOps.Index queue.

.DESCRIPTION
    Performs the expensive pass of index maintenance. Queries
    sys.dm_db_index_physical_stats for fragmentation data, updates Index_Registry
    with current fragmentation percentages, adds qualifying indexes to Index_Queue
    with priority scores, refreshes existing queue entries, and removes entries that
    no longer qualify. Combines scanning and queue population into a single living-
    queue approach.
...
.PARAMETER Force
    Override the interval check and run even if SCAN completed recently.
...
.NOTES
    File Name : Scan-IndexFragmentation.ps1
    Location  : E:\xFACts-PowerShell\Scan-IndexFragmentation.ps1
...
#             inline SQL string literals to here-strings. Renamed the local
#             Calculate-PriorityScore to Measure-idx_PriorityScore (approved verb
#             and idx_ prefix) and migrated the abort check onto the idx_-prefixed
#             shared Test-idx_AbortRequested.
...
# 2026-01-21  Table references updated to Index_* naming. GlobalConfig-based
#             settings, server-level master switch.
# 2026-01-15  Fixed queue lookup to include all statuses. Prevents duplicate queue
#             entries for the same index.
# 2026-01-14  Integrated with xFACts-IndexFunctions.ps1. Added startup abort check.
# 2026-01-13  Maintenance component refactor. Updated table references.
# 2026-01-10  Initial PowerShell implementation. Scans dm_db_index_physical_stats
#             with scaled timeouts. Maintains living queue with priority scoring.
...
<# ============================================================================
   FUNCTIONS: PRIORITY SCORING
   ----------------------------------------------------------------------------
   Composite priority-score calculation for queued indexes, combining database
   priority, fragmentation, page count, and deferral count.
   Prefix: idx
   ============================================================================ #>

# Calculates a composite priority score for an index from database priority, fragmentation, page count, and deferrals.
function Measure-idx_PriorityScore {
    param(
...
<# ============================================================================
   ...
   AG-primary resolution, the per-database fragmentation scan and queue maintenance,
   summary, and orchestrator callback.
   Prefix: (none)
   ============================================================================ #>
...
if ($Execute -and (Test-idx_AbortRequested -ServerInstance $ServerInstance -Database $Database -SettingName 'index_scan_abort')) {
...
foreach ($db in $targetDatabases) {
    ...
    if ($batchCounter -ge $batchCheckSize) {
        ...
        # Check abort flag
        if (Test-idx_AbortRequested -ServerInstance $ServerInstance -Database $Database -SettingName 'index_scan_abort') {
...
"@
        ...
        if ($fragPct -ge $dbFragThreshold -and $pageCount -ge $dbMinPageCount) {
            # Qualifies for queue
            $priorityScore = Measure-idx_PriorityScore -DatabasePriority $dbPriority `
