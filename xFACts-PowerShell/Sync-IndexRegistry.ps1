<#
.SYNOPSIS
    Synchronizes index metadata into the ServerOps.Index registry.

.DESCRIPTION
    Performs the cheap-pass reconciliation of index metadata. Discovers new indexes
    and adds them to Index_Registry, updates metadata (page count, fill factor) for
    existing indexes, captures usage statistics (seeks, scans, lookups, updates), and
    marks dropped indexes as is_dropped = 1. This does not scan for fragmentation;
    that expensive pass is handled by Scan-IndexFragmentation.ps1.
...
.PARAMETER AllDatabases
    Process all active databases regardless of the index_sync_enabled flag.
...
.PARAMETER Force
    Override the interval check and run even if SYNC completed recently.
...
.NOTES
    File Name : Sync-IndexRegistry.ps1
    Location  : E:\xFACts-PowerShell\Sync-IndexRegistry.ps1
...
#             removed the deployment block and inline dividers, converted the inline
#             SQL string literals to here-strings, and relocated a trailing comment
#             to lead its line.
...
# 2026-02-14  Orchestrator v2 standardization. Added -Execute, -TaskId, -ProcessId,
#             orchestrator callback. Converted -Force/-AllDatabases to switches,
#             added file logging. Relocated to E:\xFACts-PowerShell.
...
# 2026-01-10  Initial PowerShell implementation. Inexpensive index metadata and
...
<# ============================================================================
   IMPORTS: SCRIPT DEPENDENCIES
   ----------------------------------------------------------------------------
   Dot-sources the shared orchestrator functions.
   Prefix: (none)
   ============================================================================ #>
...
<# ============================================================================
   EXECUTION: SCRIPT EXECUTION
   ----------------------------------------------------------------------------
   The procedural execution body: server-level gate, interval check, target-database
   resolution, AG-primary resolution, the per-database index discovery and registry
   reconciliation loop, summary, and orchestrator callback.
   Prefix: (none)
   ============================================================================ #>
...
"@
    ...
    $existingLookup = @{}
    # Track which entries we've seen
    $seenKeys = @{}
    if ($existingRegistry) {
