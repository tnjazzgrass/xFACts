<#
.SYNOPSIS
    xFACts - DM Application Server Health Metrics Collection

.DESCRIPTION
    xFACts - DmOps.AppServer
    Script: Collect-DmHealthMetrics.ps1
    Version: Tracked in dbo.System_Metadata (component: DmOps.AppServer)

    Collects health metrics from all DmOps-enabled application servers in
    ServerRegistry and writes results to three tables:

    1. DmOps.App_Snapshot — One row per server per cycle. Five data sources:
    ...
    2. DmOps.App_QueueSnapshot — One row per active JMS queue per server.
       ~18 queues × 3 servers = ~54 rows per cycle.

    3. DmOps.App_ConfigHistory — Write-on-change only. Tracks JBoss config
...
Initialize-XFActsScript -ScriptName 'Collect-DmHealthMetrics' `
...
Write-Log "========================================"
Write-Log "xFACts DM Health Metrics Collection"
Write-Log "========================================"
...
if ($null -eq $servers -or @($servers).Count -eq 0) {
    Write-Log "No active servers configured for DmOps monitoring. Exiting." "WARN"
    exit 0
}
...
"@
    ...
    # ----------------------------------------
    # Step 5d: Insert App_Snapshot
    # ----------------------------------------
    $insertQuery = @"
INSERT INTO DmOps.App_Snapshot
...
"@
    ...
    if ($Execute) {
        ...
        if ($result) {
            Write-Log "  App_Snapshot inserted." "SUCCESS"
        }
        else {
            Write-Log "  Failed to insert App_Snapshot." "ERROR"
            $serverSuccess = $false
        }
    }
    else {
        Write-Log "  [Preview] Would insert App_Snapshot for $serverName"
    }
        ...
        if ($null -ne $queueResult) {
            ...
            for ($i = 0; $i -lt $ActiveQueues.Count; $i++) {
                ...
                if ($null -ne $step -and $step.outcome -eq 'success') {
                    ...
                    $qInsert = @"
INSERT INTO DmOps.App_QueueSnapshot
            ...
            for ($i = 0; $i -lt $ConfigSettings.Count; $i++) {
                ...
                if ($null -ne $step -and $step.outcome -eq 'success') {
                    ...
                    if ($null -eq $previousValue -or $previousValue -ne $currentValueStr) {
                        ...
                        $configInsert = @"
INSERT INTO DmOps.App_ConfigHistory
...
if ($TaskId -gt 0) {
    ...
    $outputMsg = "Servers: $successServers/$serverCount, Queues: $totalQueueRows rows, Config: $totalConfigRows changes"
    if ($failedServers.Count -gt 0) {
