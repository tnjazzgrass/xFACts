...
>
> **This document contains Block 1 and Block 2 planning notes from the initial B2B build.** It reflects the pre-investigation architectural view (MAIN-centric, assumed-incomplete retention, narrow scope).
>
> **Authoritative sources:** `Planning/B2B_Roadmap.md` and `WorkingFiles/B2B_Investigation/` step findings.
>
> The planning decisions captured here led to the current `SI_ExecutionTracking` and `SI_ScheduleRegistry` tables and `Collect-B2BExecution.ps1` collector. Those artifacts remain functional for their narrow scope but are under re-evaluation as part of the full investigation reset. This document may contain outdated framing; trust the Roadmap and step findings over anything here.
