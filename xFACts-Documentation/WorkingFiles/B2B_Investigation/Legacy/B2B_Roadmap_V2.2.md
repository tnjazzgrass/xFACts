...
**Status:** Active — investigation phase (Steps 1-5 complete, Step 6A complete, Step 6B next)
**Version:** 2.2
**Last updated:** 2026-04-24
**Supersedes:** v2.1 (updated in-session); v2.0; v1 (archived at `WorkingFiles/B2B_Investigation/Legacy/B2B_Roadmap_V1.md`)
...
Step 6A catalogued the full workflow universe (1,433 WFDs, 413 active in 30 days). Step 6B extracts the BPML XML source for ~430 workflows (413 active + 17 dormant FA_CLIENTS that run inline inside MAIN). These BPMLs are the authoritative structural source for every workflow — reading them replaces runtime-inference with source-code verification.

### Approach (Step 6 overall)

Step 6 is **split into checkpointed sub-steps** (6A through 6G). Each produces its own findings document and is followed by a Roadmap update before the next begins. High-level sequence:

- **6A — Active Workflow Catalog** ✅ Complete
- **6B — BPML Bulk Extraction** 🎯 Next
- **6C — Core Workflow BPML Analysis** (FA_CLIENTS family + representative dispatchers)
- **6D — Claim Verification against ArchitectureOverview**
- **6E — Runtime Verification** (queries against b2bi for claims BPML can't verify)
- **6F — Light Catalog of remaining BPMLs** (all workflows not deep-read in 6C)
- **6G — Consolidation** (single Step 6 Summary; claim checklist resolved; ArchitectureOverview reconciled)

### Session start prompt template

When opening the next session:

> Starting Step 6B — BPML Bulk Extraction. Cache-busted manifest URL: https://raw.githubusercontent.com/tnjazzgrass/xFACts/main/manifest.json?v=<value>

### Required context before starting 6B

1. **This Roadmap** (you're reading it) — current state, Known True list, pending decisions
2. **`WorkingFiles/B2B_Investigation/Step_06_MAIN_Anatomy/Step_06A_Active_Workflow_Catalog/Step_06A_Findings.md`** — the extraction target list and metadata
3. Schema discovery will be the first 6B task — locate the b2bi table(s) storing BPML XML (not `WFD.WFD_XML` — that column does not exist; likely a separate `WFD_XML` table or `DATA_TABLE` indirection similar to SCHEDULE.TIMINGXML)
...
**Status:** 🎯 Active — Step 6A complete, 6B next
**Findings so far:** `Step_06_MAIN_Anatomy/Step_06A_Active_Workflow_Catalog/Step_06A_Findings.md`
...
- **6A — Active Workflow Catalog** ✅ Complete. 1,433 WFDs catalogued, 413 active in 30d, extraction target list of ~430 BPMLs identified.
- **6B — BPML Bulk Extraction** 🎯 Next. Discover the b2bi table(s) storing BPML XML; extract all ~430 target BPMLs; organize them by family on disk; catalog size/extract-success metrics.
- **6C — Core Workflow BPML Analysis.** Deep-read the FA_CLIENTS family (all 28 active + dormant) plus representative dispatchers (FA_FROM/FA_TO Pattern 1/3/4 examples). Document each workflow's sequence, rules, sub-workflow invocations (with INVOKE_MODE), service calls, fault handlers, and external executable references.
...
**What we know:** ARCHIVE runs 10,167/30d; VITAL runs 10,179/30d; EMAIL 620/30d; ENCOUNTER_LOAD 82/30d (with 6.1% fail rate — highest among FA_CLIENTS). Their BPMLs will be read in Step 6C alongside MAIN. Standalone runtime behavior analysis (failure modes across runs, content of their outputs) comes after Step 6 completes.
...
**Candidates:** `ACT_XFER` (33K rows), `ACT_NON_XFER` (48K rows), `ACTIVITY_INFO` (33K rows), `DATA_FLOW` (30K rows), `ACT_SESSION` (17K rows), `DOCUMENT` (348K rows), `DOCUMENT_EXTENSION` (235K rows), `SCHEDULE` (605 rows; already known via `SI_ScheduleRegistry`), `SERVICE_INSTANCE`, `SERVICE_DEF`, `SERVICE_PARM_LIST`. Plus: a yet-to-be-discovered table storing BPML XML (target of Step 6B schema discovery).
...
### 5.15 Open questions raised by Step 6A (to be resolved in 6C-6E)
...
6. **`Step_06_MAIN_Anatomy/`** — umbrella folder for Step 6 sub-steps:
   ...
   - `Step_06B_BPML_Bulk_Extraction/` — next
   - `Step_06C_Core_Workflow_BPML_Analysis/` — to follow
...
1. **🎯 Step 6B — BPML Bulk Extraction (§5.6).** Next up. Schema discovery first (locate BPML storage table), then PowerShell extraction of ~430 BPMLs.
2. **Step 6C — Core Workflow BPML Analysis.** Deep-read FA_CLIENTS family + representative dispatchers.
3. **Step 6D — Claim Verification.** Extract and resolve every ArchitectureOverview claim.
4. **Step 6E — Runtime Verification.** Runtime queries for claims BPML can't answer.
5. **Step 6F — Light BPML Catalog.** The remaining ~390 non-FA_CLIENTS active BPMLs.
6. **Step 6G — Consolidation.** Step 6 summary; claim checklist resolved; ArchitectureOverview reconciled.
7. **Post-Step-6 steps** — sub-workflow runtime deep-dives (ARCHIVE, VITAL, EMAIL, ENCOUNTER_LOAD); dispatcher runtime analysis; process type semantic verification (§5.9); Integration DB verification (§5.10); remaining table investigation (§5.12).
8. **Decision phase.** §7.x resolved; collector architecture defined.
