> ### ⚠️ Deprecated — Reference Only (mostly trustworthy)
>
> **This document contains SQL queries validated against b2bi and Integration during the initial B2B build.** Most queries remain technically correct — they were actually run and produced results.
>
> **However:** the queries were designed around a MAIN-only scope and may not reflect the full workflow universe. Queries that join `WF_INST_S` to `WFD` on `WFD_ID` alone are at risk of version-multiplication bugs (Step 3 findings have details).
>
> **Authoritative sources:** `Planning/B2B_Roadmap.md` and `WorkingFiles/B2B_Investigation/` step findings and per-step SQL.
>
> Use queries from this document as starting points, but verify column names and join conditions against current step findings before relying on results.
