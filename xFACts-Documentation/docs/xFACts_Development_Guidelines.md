...
Components are defined in `dbo.Component_Registry`. The current list is exported to `Component_Registry.md` (under `xFACts-Generated/registry/`) by the publisher and should be available in session context. This is the authoritative reference for component names, module membership, and descriptions -- always check it before querying the database. Query `Component_Registry` directly only if the export is not available:
...
- `column_name`: NULL
- `content`: Module name -- must match an active entry in `dbo.Module_Registry`. Refer to `Module_Registry.md` (under `xFACts-Generated/registry/`) for the current list, or query `SELECT module_name FROM dbo.Module_Registry WHERE is_active = 1` if the file is unavailable.
...
**Publishing:** `Publish-GitHubRepository.ps1` handles all publishing. It collects files from the server source directories, extracts SQL object definitions from the database, generates the per-table registry exports (`xFACts-Generated/registry/`) and per-schema Object_Metadata exports (`xFACts-Generated/metadata/`), audits all files against Object_Registry, compares the local inventory against the current repo state, and pushes only changed files. A `manifest.json` is generated and pushed as the final step. The script runs standalone or as a step in the `Invoke-DocPipeline.ps1` pipeline (triggered from the Admin page Documentation modal).
...
| `xFACts-Documentation/WorkingFiles/*` | Working reference files for active builds, not platform objects |
| `xFACts-Documentation/docs/*.md` | Authored reference documents (Guidelines, Backlog) |
| `xFACts-Generated/ddl/*.json` | Generated output of Generate-DDLReference.ps1 |
| `xFACts-Generated/md/*` | Generated module markdown export |
| `xFACts-Generated/registry/*` | Per-table registry exports, runtime-generated |
| `xFACts-Generated/metadata/*` | Per-schema Object_Metadata exports, runtime-generated |
| Generated files (manifests) | Runtime-generated, not authored objects |

`Legacy`-schema SQL objects are not an audit exclusion: they are excluded upstream at extraction (`Get-SqlObjectDefinitions` in `xFACts-DocPipelineFunctions.ps1`) and never reach the publish set or the audit.
...
|----------|-----------------|---------|
| PowerShell (`manifest-powershell`) | `E:\xFACts-PowerShell\` | `.ps1` and `.psm1` scripts and shared functions (flat directory) |
| Control Center App (`manifest-cc-app`) | `E:\xFACts-ControlCenter\` (excluding `public\docs`) | Route files, API files, modules, CSS, JS, startup |
| Control Center Docs (`manifest-cc-docs`) | `E:\xFACts-ControlCenter\public\docs\` | Documentation-site HTML pages, docs CSS/JS |
| Documentation (`manifest-documentation`) | `E:\xFACts-Documentation\docs`, `\Planning`, `\WorkingFiles` | Guidelines, backlog, planning docs, working files |
| Generated (`manifest-generated`) | `E:\xFACts-Generated\` and database extraction | DDL JSON, module markdown, SQL object definitions, per-table registry exports, per-schema Object_Metadata exports |
