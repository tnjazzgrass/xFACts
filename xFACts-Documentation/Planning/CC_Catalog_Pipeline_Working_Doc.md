...
- `Populate-AssetRegistry-JS.ps1` — current. Updated across Session 4 (2026-05-16) to align with the JS spec amendments. Recognizes BOOTLOADER section type; INITIALIZATION removed from page-file allowed types; `DUPLICATE_BOOTLOADER`, `MISSING_PAGE_INIT`, `ENGINE_PROCESSES_MISPLACED`, `HOOK_MISPLACED` detection added. Emits `JS_DISPATCH_ENTRY` rows for each entry in `<prefix>_<event>Actions` (page-side) and `shared<event>Actions` (cc-shared.js side). Cross-references `ENGINE_PROCESSES` declarations against `Orchestrator.ProcessRegistry` with four validation drift codes. Cross-references HTML_ID USAGE rows against HTML_ID DEFINITION rows from the HTML populator (`JS_HTML_ID_UNRESOLVED`, `JS_HTML_ID_MALFORMED`). Implements the contract identifier framework per JS spec §5.5: `Test-IsContractIdentifier` helper and the `$ContractIdentifiers` set exempt `ENGINE_PROCESSES` and the five hook names from `PREFIX_MISSING` / `PREFIX_MISMATCH`. Otherwise the populator is sound: it consumes `xFACts-AssetRegistryFunctions.ps1`, walks via `Invoke-AstWalk`, uses the pre-built section list, performs prefix registry validation with strict-with-carve-outs, and splits banner detection into permissive admission plus strict validation. Universal anchor refactor 2026-05-12: emits `Add-JsFileRow` as the pure-anchor row builder. **Performance:** ~7.5 min runtime on full 31-file scan (vs ~80 sec for next-slowest populator). Investigation pending; see "JS populator performance investigation" below.
...
Strategic framing as of Session 4 (2026-05-16): three populators (CSS, JS, PS) are at parity with current spec knowledge. The HTML populator is the only one lagging. Rather than continue staged "wave-by-wave" populator work, the plan is to bring HTML to the same level in a single completion pass, then begin page-by-page Phase 1 migrations. Phase 1 conversions surface real-file drift as authoritative evidence for any further spec or populator amendments; speculation about edge cases without real refactored files has repeatedly proven unreliable.

Active priorities for next session(s):

1. **HTML populator completion (next-session active priority).** Bring the HTML populator to parity with CSS/JS/PS — complete to current spec knowledge in a single focused push. All previously-planned wave items (Wave 2.1 drift attachment, Wave 3 HTML_TEXT/HTML_ENTITY/HTML_SVG/HTML_COMMENT extraction, the bootloader-driven spec amendments — `data-page`/`data-prefix` recognition, `data-action-<event>` family, `#page-error-banner` placeholder, `FORBIDDEN_INLINE_EVENT_HANDLER` umbrella plus 16 shape codes, `HTML_EVENT_HANDLER` rows, dispatch-value cross-spec resolution against JS_DISPATCH_ENTRY rows) folded together. This is the gating prerequisite for Phase 1 migrations.

2. **Phase 1 migrations — page-by-page skeleton refactor.** Once HTML populator is at parity, begin per-page Phase 1 conversions. Phase 1 = minimal refactor to get a page operating on the new shared files (`cc-shared.js`, `cc-shared.css`) with bootloader-driven boot, contract identifiers in their required homes, and spec-form file/banner/section structure. All existing behavior preserved; everything that violates the spec but doesn't break the page surfaces as drift. Full playbook in `CC_Migration_Phase1.md`. First page selection deferred to that conversion session.

3. **JS populator performance investigation.** ~6x per-row slowdown vs other populators is acceptable for dev-cycle catalog builds but unacceptable for the planned Admin-tile interactive operation. Investigation deferred until Phase 1 work begins; details and investigation paths captured in "JS populator performance investigation" section below.

4. **`Refresh-AssetRegistry.ps1` orchestrator.** Cross-populator orchestrator: single TRUNCATE, then dispatch CSS → HTML → JS → PS in order, with `sp_getapplock` single-instance locking, on-demand execution from Admin page, and consolidated logging. Sequencing-wise can land any time after all populators are current.

5. **engine-events retirement.** As Phase 1 conversions complete, each converted page stops using `engine-events.js` and `engine-events.css`. When every page is converted, delete both files plus `engine-events-API.ps1`.

6. **`docs-base.css` → `docs-shared.css` migration.** Parallel to `engine-events.css` → `cc-shared.css`. Anchor file for `Documentation.Site`. Includes a small populator change to recognize the second anchor file. Independent of the page migration sequence.

7. **Per-file refactor for the seven docs CSS files.** Joins the per-file refactor queue. Independent of the page migration sequence.
...
### JS populator performance investigation

The JS populator is significantly slower per file than the other three populators. Pipeline-level measurements taken 2026-05-16 (first complete run of all four populators after the bootloader/dispatch/ENGINE_PROCESSES/contract-identifier amendments):

| Populator | Files | Rows | Total runtime | Parse time | Walk time | Rows/sec | s/file (walk) |
|---|---|---|---|---|---|---|---|
| CSS  | 33 |  7,673 |  69 s |  21 s |  42 s | 111 | 1.27 |
| HTML | 41 |  3,080 |  10 s |   0 s |   8 s | 308 | 0.20 |
| **JS**   | 31 | 10,954 | **457 s** | 107 s | 342 s | **24** | **11.03** |
| PS   | 86 | 12,151 |  79 s |   2 s |  71 s | 154 | 0.83 |

JS is producing 353 rows per file (vs PS at 141 and CSS at 233) — more work per file by ~1.5–2x — but the per-row throughput shows the real disparity: JS is ~6x slower per row than PS, ~5x slower than HTML, and ~4.6x slower than CSS. The 7.5-minute runtime is acceptable for catalog-build operations today but will not be acceptable once the pipeline launches from an Admin-tile control (screen-locked operation, expected 3-4 invocations per day during active development).

Suspected contributors, in roughly decreasing order of probability:

1. **Node subprocess overhead per file.** The JS populator shells out to `node parse-js.js` for every `.js` file (~3.45 s/file in Pass 1). Process creation, V8 startup, Acorn load, parse, JSON serialize, stdin/stdout marshaling, teardown. CSS uses the same pattern but loads PostCSS (less startup overhead than Acorn). HTML and PS use in-process PowerShell parsers with no subprocess at all.
2. **Visitor walking via PowerShell over deserialized JSON.** The JS visitor walks PSCustomObjects deserialized from Node's JSON output. Every property access on a PSCustomObject in PowerShell is comparatively slow vs accessing typed .NET objects (which the PS populator gets natively from `[System.Management.Automation.Language.Parser]`).
3. **Repeated `PSObject.Properties.Name -contains` checks** in helper functions (`Test-IsInsideElementLoop`, `Test-CalleeMatchesEnd`, `Get-CurrentParentName`, etc.). Linear scans of a property-name array, called frequently in the visitor's hot path.
4. **Per-emission section lookup.** `Get-SectionForLine` may be doing linear scans through the section list at every definition emission. ~30 banners per file × ~350 emissions per file ≈ 10K linear scans.

Investigation paths, roughly ordered by likely return-on-effort:

- **Batched Node parsing.** Send all 31 files to one Node process via a stdin/stdout protocol, parse them all in one Node lifetime. Eliminates ~3 s/file of spawn cost. Estimated savings: 60–90 seconds (~13–20% of total runtime). Implementation: protocol design plus `parse-js.js` rewrite into a long-running reader. Moderate effort.
- **Pre-sorted section index with binary search.** Replace any linear section-lookup with a sorted line→section index and binary search. Estimated savings: hard to know without measurement; possibly modest.
- **Typed-object conversion at parse time.** Convert JSON → typed PowerShell custom classes once at deserialization, walk fast typed objects after. Risky refactor; could help substantially or could be a wash. Worth scoping but not undertaking without measurement first.
- **Profile before optimizing.** Run the JS populator under `Measure-Command` against representative files (small file, medium file, cc-shared.js) and break out per-phase costs (parse vs walk vs row construction vs registry preload). The numbers above are aggregate; finer-grained measurements would let us target the biggest single contributor first rather than guessing.

**Deferred until:** all bootloader/dispatch/contract-identifier work is complete, the spec files have been refactored to confirm the populator behaves correctly on the new format, and at least one Phase 1 page conversion has completed end-to-end. At that point the populator behavior is well-validated and the optimization work can be undertaken without conflating performance fixes with correctness fixes.
...
- **2026-05-16 (Session 4)** -- JS spec amendments shipped: §5.5 (contract identifiers framework), §7.4.3 (ENGINE_PROCESSES placement), §8.5 (hook function placement), updates to §8.4 and §19.3. New drift codes `ENGINE_PROCESSES_MISPLACED` and `HOOK_MISPLACED` (the `_MISPLACED` family). Appendix A.5.5 added; A.8 updated.
- **2026-05-16 (Session 4)** -- JS populator updated to current spec across two rounds. Round 1: BOOTLOADER recognition, INITIALIZATION removal, MISSING_PAGE_INIT, JS_DISPATCH_ENTRY emission (one row per dispatch-table entry), ENGINE_PROCESSES validation against `Orchestrator.ProcessRegistry`, HTML_ID cross-spec resolution. Round 2: bug fixes (3-char prefix regex hardcoding, var vs const ENGINE_PROCESSES capture) plus contract identifier framework — `$ContractIdentifiers` set, `Test-IsContractIdentifier` helper, prefix carve-outs at four PREFIX_MISSING sites and three PREFIX_MISMATCH sites, `ENGINE_PROCESSES_MISPLACED` and `HOOK_MISPLACED` detection.
- **2026-05-16 (Session 4)** -- First full-pipeline run (all four populators in sequence: CSS → HTML → JS → PS) completed against the full Control Center codebase. Total runtime 10 minutes 18 seconds across 191 source files producing 33,858 rows. JS populator identified as ~6x slower per row than other populators; performance investigation deferred until Phase 1 page conversions begin.
- **2026-05-16 (Session 4)** -- Strategic shift to per-page migration approach. Wave-by-wave per-file-type populator development replaced by "bring HTML populator to parity in one pass, then begin Phase 1 page conversions." Rationale: speculation about edge cases without real refactored files has repeatedly surfaced spec/populator gaps in unproductive cycles. Per-page conversion provides authoritative validation. Phase 1 playbook (skeleton refactor only) captured in `CC_Migration_Phase1.md`. Page selection order deferred to first conversion session.
