...
Catalog is functional and queryable. Production populator scripts are deployed and running successfully against the full Control Center codebase plus the docs site CSS files. HTML spec locked 2026-05-10 and amended 2026-05-11 (HTML_FILE anchor row, read-only cross-populator model). All schema additions implied by the HTML spec are deployed (`Asset_Registry.has_dynamic_content`, four `ProcessRegistry` cc-prefixed columns, `CK_Asset_Registry_component_type` admits `HTML_FILE`). Helpers file patched 2026-05-11 for `has_dynamic_content` support. Production HTML populator build is the next major work item, scheduled for a fresh session.
...
- `Populate-AssetRegistry-HTML.ps1` — pre-build, scheduled for next session. The existing test populator (`xFACts-Documentation/WorkingFiles/TEST-Populate-AssetRegistry-HTML.ps1`) is a sketch only; it predates the HTML spec and doesn't reflect the locked-and-amended spec design. Production HTML populator is a fresh build informed by the spec rather than a port of the test populator. The build consumes `xFACts-AssetRegistryFunctions.ps1` (patched 2026-05-11 for `has_dynamic_content` support), follows the visitor pattern, performs prefix registry validation, adopts permissive-admission/strict-validation, walks PS files via `[System.Management.Automation.Language.Parser]::ParseFile()`, emits the `HTML_FILE` anchor row plus all HTML construct rows per `CC_HTML_Spec.md` §13 (amended 2026-05-11), and emits the 88 drift codes per §15. Schema is in place — see "Where we left off" item 1 for the full build requirements list.

**Shared infrastructure (deployed):**

- `xFACts-AssetRegistryFunctions.ps1` — domain-specific helpers file for the Asset Registry populator family. ~1,295 lines, 22 functions after the 2026-05-07 banner-detection split; patched 2026-05-11 to wire `has_dynamic_content` through `New-AssetRegistryRow` and `Invoke-AssetRegistryBulkInsert`. Centralizes row construction, dedupe tracking, drift code attachment (hybrid: master-table validation plus optional row-specific context), occurrence-index computation, Object_Registry / Component_Registry registry loads, bulk insert plus DataTable shape, comment-text cleanup, banner detection (permissive `Test-IsBannerComment` plus strict `Get-BannerInfo`), file-header parsing, pre-built section list construction with body-line ranges, file-org match check, and the generic AST visitor walker. Pattern parallels `xFACts-IndexFunctions.ps1`: each populator dot-sources `xFACts-OrchestratorFunctions.ps1` first, then `xFACts-AssetRegistryFunctions.ps1`, then calls `Initialize-XFActsScript`. Deployed alongside the refactored CSS and JS populators. The HTML populator build will consume the same helpers file; if any HTML-specific helper functions are needed, they will be added there following the same pattern (e.g., HTML attribute extraction, page-prefix stripping for category derivation per `CC_HTML_Spec.md` §8.2.2).
...
Sessions are organized around populator and spec work. Pickup options below assume the prefix registry migration (described in the Initiative doc), the 2026-05-07/2026-05-08 spec amendments, the 2026-05-09 §12 event-handler-binding amendment plus four Phase 1 page JS file refactors, the 2026-05-09 JS populator catch-up for `FORBIDDEN_PER_ELEMENT_LISTENER_LOOP`, the 2026-05-10 HTML spec drafting, and the 2026-05-11 HTML spec amendments / helpers patch are complete.

1. **Build production `Populate-AssetRegistry-HTML.ps1`.** Fresh build against the locked-and-amended HTML spec — the existing test populator is a sketch only and is not the basis for production work. Pre-build prep was completed 2026-05-11; the build itself is scheduled for a fresh session. The build:
   - Consumes `xFACts-AssetRegistryFunctions.ps1` for row construction, dedupe, drift attachment, registry loads, bulk insert, banner detection, file-header parsing, section list construction, and the visitor walker. Helpers file was patched 2026-05-11 to wire `has_dynamic_content` through the row builder and bulk-insert DataTable.
   - Tokenizes PS files via `[System.Management.Automation.Language.Parser]::ParseFile()` to extract here-string and string token contents (no external dependency). PS-AST walking logic is new to the populator family — the existing `Invoke-AstWalk` in the helpers file is shaped for JS/CSS AST, not PS AST; the HTML populator implements its own PS-AST walker.
   - Emits the `HTML_FILE` anchor row per scanned PS file (one row per file with HTML emission; route files get `scope=LOCAL`, helper files get `scope=SHARED`; hosts the §15.1 page-shell drift codes for route files).
   - Walks HTML markup inside extracted string tokens to identify the HTML constructs cataloged per `CC_HTML_Spec.md` §13 — DEFINITION rows for `HTML_ID`, `HTML_DATA_ATTRIBUTE`, `HTML_TEXT`, `HTML_ENTITY`, `HTML_SVG`, `HTML_COMMENT`, plus USAGE rows for `CSS_CLASS`, `JS_FUNCTION`, `CSS_FILE`, `JS_FILE` from class attributes, event handlers, and asset references.
   - Resolves USAGE rows against existing DEFINITION rows in the catalog (per pipeline order, CSS rows always exist before HTML scans). Read-only cross-populator model per §13.6 (amended 2026-05-11): the HTML populator reads from upstream DEFINITION rows, never edits them. `parent_function` for HTML rows is filled directly by the HTML populator from its own PS-AST walk's enclosing function context — no cross-populator write-back.
   ...
   - Implements `has_dynamic_content` flag attachment per spec §5.5 and §13.5 — set TRUE on class attribute and text node rows where the parent construct contains PS interpolation the populator cannot statically resolve.
   - Implements engine card validation against `Orchestrator.ProcessRegistry` cc-prefixed columns per spec §2.3.
   - Emits the 88 drift codes per `CC_HTML_Spec.md` §15.
   - Schema is in place: `Asset_Registry.has_dynamic_content` is deployed; `ProcessRegistry.cc_engine_slug`, `cc_engine_label`, `cc_page_route`, `cc_sort_order` are deployed; `CK_Asset_Registry_component_type` admits `HTML_FILE`. DDL JSON regeneration is the only remaining prerequisite — the populator will reference the regenerated JSON to validate column names against current schema.
...
The four populators run in a fixed order driven by the cross-population dependency relationships. CSS is the grandparent — produces `CSS_CLASS DEFINITION` rows that HTML and JS resolve against. HTML is the parent — produces `HTML_ID DEFINITION` and `HTML_DATA_ATTRIBUTE DEFINITION` rows that JS resolves against, and depends on CSS for class scope resolution. JS is both child and grandchild — depends on CSS for class scope resolution and on HTML for ID and data-attribute resolution. PS runs last; it emits its own per-file anchor row distinct from HTML's `HTML_FILE` anchor row (different purposes, same physical file).
...
The 2026-05-11 §13.6 amendment locked in a strict read-only cross-populator model: no populator ever edits another populator's rows; all cross-references resolve at scan time against existing DEFINITION rows produced by upstream populators. An earlier model implied that the PS populator would enrich existing HTML rows by filling in `parent_object` values; that model is retired. Each populator is independently re-runnable, and the populator family invariant "produce rows, never modify others' rows" holds uniformly.
...
- **`has_dynamic_content`** (BIT NULL, added 2026-05-10) — flag column for partial extraction. Set TRUE on rows where the parent attribute or text construct contains additional runtime-only content the populator cannot statically resolve. Applies to HTML and JS populator rows; CSS rows leave it NULL. Helpers file wired 2026-05-11 to support the column through `New-AssetRegistryRow` and `Invoke-AssetRegistryBulkInsert`.
...
| `HTML_FILE` | File-level anchor for HTML emission. One row per scanned PS file with HTML emission. Hosts §15.1 page-shell drift codes for route files. | HTML |
...
`CK_Asset_Registry_component_type` CHECK constraint admits 43 values total (added `HTML_FILE` on 2026-05-11).
...
The schema below reflects what is currently live in `dbo.Asset_Registry` and `Orchestrator.ProcessRegistry`. The `has_dynamic_content` column and the four `ProcessRegistry` cc-prefixed columns are deployed as of the HTML spec drafting session (2026-05-10) and helpers file patch (2026-05-11). `CK_Asset_Registry_component_type` admits 43 component types as of 2026-05-11 (including `HTML_FILE`).
...
**Constraints:** CHECK on `file_type` in {CSS, JS, PS, HTML}, CHECK on `component_type` in enumerated list (43 values as of 2026-05-11), CHECK on `reference_type` in {DEFINITION, USAGE}, CHECK on `scope` in {SHARED, LOCAL}.
...
| 1B | HTML extraction from .ps1/.psm1 string tokens | PRE-BUILD — HTML spec locked 2026-05-10, amended 2026-05-11; production populator build scheduled for next session; pre-build prep complete (schema deployed, helpers patched, HTML_FILE component type and read-only cross-populator model locked) |
...
### HTML populator — fresh build against locked-and-amended spec

HTML spec locked 2026-05-10, amended 2026-05-11. Production HTML populator is a fresh build, not a port of the test populator. Build requirements:

- Consume `xFACts-AssetRegistryFunctions.ps1` for shared infrastructure (patched 2026-05-11 for `has_dynamic_content` support).
...
- Implement a PS-AST walker — the existing `Invoke-AstWalk` in the helpers file is JS/CSS-AST-shaped, not PS-AST-shaped. The HTML populator's PS-AST walker tracks the enclosing function context so `parent_function` is filled at row emission from the AST walker's state.
- Implement the permissive-admission/strict-validation pattern for HTML constructs with defined shapes (page chrome elements, ID conventions, class attribute compositions, event handler shapes).
- Emit the `HTML_FILE` anchor row per scanned PS file with HTML emission. One row per file; route files get `scope=LOCAL` with `component_name` = page route; helper files get `scope=SHARED` with `component_name` = helper function name. Hosts §15.1 page-shell drift codes for route files; §15.1 codes do not fire on helper files.
- Emit the 88 drift codes per `CC_HTML_Spec.md` §15.
- Resolve `CSS_CLASS USAGE` rows against existing `CSS_CLASS DEFINITION` rows; resolve `CSS_FILE` and `JS_FILE` USAGE rows against their respective DEFINITION rows. Read-only cross-populator model per §13.6 (amended 2026-05-11): never edit rows produced by other populators.
...
Schema is in place. All schema additions implied by the HTML spec are deployed:

- `dbo.Asset_Registry.has_dynamic_content BIT NULL` — deployed; helpers file patched 2026-05-11 to support it.
- `Orchestrator.ProcessRegistry.cc_engine_slug VARCHAR(20) NULL` — deployed.
- `Orchestrator.ProcessRegistry.cc_engine_label VARCHAR(50) NULL` — deployed.
- `Orchestrator.ProcessRegistry.cc_page_route VARCHAR(100) NULL` — deployed.
- `Orchestrator.ProcessRegistry.cc_sort_order INT NULL` — deployed; content backfill pending.
- `CK_Asset_Registry_component_type` CHECK constraint — admits `HTML_FILE` (added 2026-05-11). Admits 43 values total.

DDL JSON regeneration (via `Generate-DDLReference.ps1`) is the only remaining prerequisite before the populator build session, so the populator validates column names against the current schema.
...
5. Run JS populator.
6. (When PS populator lands) run PS populator. The PS populator emits its own per-file anchor row distinct from HTML's `HTML_FILE` row; no cross-populator write-back to HTML rows.
7. Release applock.
...
The 2026-05-11 §13.6 amendment crystallized a design principle that had been informally true and then drifted: no populator ever edits another populator's rows. Cross-references resolve at scan time against existing DEFINITION rows produced by upstream populators, and the resolution is a read. The earlier model implied that the PS populator would enrich existing HTML rows by filling in `parent_object` values after the HTML populator's run; that model is retired.

Every populator is independently re-runnable under the new model. The populator family invariant -- "produce rows, never modify others' rows" -- holds uniformly. Each populator's responsibility is its own slice of `Asset_Registry` (delimited by `file_type` or, for HTML rows, by the producing populator's claim on the row set). Cross-populator data flow happens at scan time, not at write-back time. The clean separation makes the dependency chain (CSS -> HTML -> JS -> PS) sufficient: every populator reads from upstream DEFINITION rows already in the catalog, and that's the whole protocol.

The lesson generalizes to future cross-cutting features: anywhere a downstream populator would otherwise enrich upstream rows, the upstream populator should produce the data itself during its own pass (looking sideways if needed, but not asking a downstream populator to fill it in later). For the HTML populator, this means `parent_function` is filled from the populator's own PS-AST walk, not from waiting for the PS populator to discover function boundaries and back-edit.
...
- **2026-05-11** -- HTML spec amended ahead of HTML populator build. §13.6 rewritten to lock in strict read-only cross-populator model (no populator edits other populators' rows; all cross-references resolve at scan time against existing DEFINITION rows). §13.1, §13.2, and §14 add the `HTML_FILE` component type -- one row per scanned PS file with HTML emission, hosting §15.1 page-shell drift codes. §5.3 removes the `parent_object` row from the CSS_CLASS USAGE column table (the column does not exist on the live schema). §14 removes a stale reference to "route path (later filled in by PS populator)" -- the PS populator no longer writes back to HTML rows. §15.1 introduces the §15.1 codes attaching to the new `HTML_FILE` anchor row. `CK_Asset_Registry_component_type` updated the same session to admit `HTML_FILE`; constraint now admits 43 values total.
- **2026-05-11** -- Helpers file (`xFACts-AssetRegistryFunctions.ps1`) patched for `has_dynamic_content` column support. Four surgical edits: `New-AssetRegistryRow` gains `[Nullable[bool]]$HasDynamicContent = $null` parameter; row hashtable gains `HasDynamicContent` field; `Invoke-AssetRegistryBulkInsert` DataTable gains `has_dynamic_content` BIT column; cell-write line added with DBNull handling for `$null`. CSS and JS populators continue working unchanged (parameter defaults to `$null`, which becomes DBNull at insert time, matching the spec's "FALSE or NULL = fully captured" semantics). HTML populator is the first consumer.
- **2026-05-11** -- HTML populator build scheduled for fresh session. Pre-build prep completed in this session: schema verified against the live state (Asset_Registry has 25 columns; no `parent_object`; CHECK constraint admits all needed component types including `HTML_FILE`; four `ProcessRegistry` cc-prefixed columns in place but content backfill pending); spec corrections applied to §5.3, §13.1, §13.2, §13.6, §14, §15.1 of `CC_HTML_Spec.md`; `HTML_FILE` decision locked in and CHECK constraint updated; helpers file patched for `has_dynamic_content`. The populator itself -- estimated 2,200-2,800 lines following the CSS/JS populator template pattern -- was deferred to a fresh session rather than written under tight context budget. Quality-over-speed call. Next-session prerequisites: regenerate DDL JSON via `Generate-DDLReference.ps1`; push spec and helpers file changes to GitHub; provide fresh cache-busted manifest URL.
