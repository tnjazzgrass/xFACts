...
- `Populate-AssetRegistry-JS.ps1` — spec-aware rewrite delivered 2026-05-04 against `CC_JS_Spec.md` v1.0. New row types (FILE_HEADER, COMMENT_BANNER, JS_STATE, JS_HOOK, JS_TIMER); section-aware drift detection covering ~25 codes; purpose_description capture on every cataloged definition; cross-file shadowing detection. Validation run produced 8,360 rows with confirmed correct CSS_CLASS USAGE scope resolution when run after the CSS populator. **Pending changes** (next session): variant model wiring per Section 17.5 of `CC_JS_Spec.md` v1.2 (including the new `JS_HOOK_VARIANT` and `JS_CONSTANT_VARIANT expression` cases, plus `FORBIDDEN_ANONYMOUS_FUNCTION` detection); `parent_function` AST-walker threading; louder pre-load reporting when CSS_CLASS DEFINITION rows are missing; remove all text-trimming from row builder so the populator writes verbatim source content into widened columns.
...
1. **JS populator change pass (next session's opener)** — apply the variant model + parent_function + pre-load reporting + trim-removal changes to `Populate-AssetRegistry-JS.ps1`. Pre-work landed in `CC_JS_Spec.md` v1.2 (variant grid in Section 17.5 plus new `FORBIDDEN_ANONYMOUS_FUNCTION` rule in Section 14/14.1) and this doc (master component_type list updated with `JS_HOOK_VARIANT`; schema column widths updated). The `ALTER_AssetRegistry_PopulatorChangePass.sql` script must run before the new populator (extends the `component_type` CHECK constraint to accept `JS_HOOK_VARIANT` and widens `component_name`, `variant_qualifier_2`, and `source_section`). Detailed change list:

   - **Variant emission:** add helper functions `Get-FunctionVariantShape`, `Get-ConstantVariantShape`, `Get-MethodVariantShape`, `Get-HookVariantShape`, `Get-TimerVariantShape`, `Get-ImportVariantShape`, `Get-EventVariantShape` modeled after the CSS populator's `Get-VariantShape`. Each helper returns `@{ ComponentType, VariantType, VariantQualifier1, VariantQualifier2 }` for its component family. Thread the result into the row builder via new parameters on `New-AssetRow` / `Add-JsDefinitionRow`. Update the bulk-insert DataTable to include the three variant columns. Centralize the component_type list and per-component variant_type values as data at the top of the file, not inline in the visitor — design rationale: spec is still being defined, multiple iterations expected through Phase 1, so amendments should be config edits not surgery.
   - **`parent_function` wiring:** thread enclosing-function-name context through `Invoke-AstWalk`. Visitor receives `($Node, $ParentChain, $ParentFunctionName)`. The walker maintains the stack with these rules: regular function declaration → push function name; function/arrow expression assigned to a const/var → push the const/var name (not `<anonymous>`); class declaration → push class name; method definition → push method name when descending into method body (the METHOD row itself reads class name from one frame up); allowed-callback anonymous functions (Section 14.1) → push the outer call's name (e.g. `'.then'`, `'addEventListener'`); pop on exit. Every USAGE row stamps `parent_function` from the current top-of-stack value. For `JS_METHOD` and `JS_METHOD_VARIANT` rows themselves, `parent_function` carries the enclosing class name.
   - **`FORBIDDEN_ANONYMOUS_FUNCTION` detection:** new drift code per `CC_JS_Spec.md` v1.2 Section 14. Fires when a `FunctionExpression` or `ArrowFunctionExpression` has no name AND its parent context is not an allowed callback context (Section 14.1). Allowed contexts: argument to a `CallExpression`. Forbidden contexts: assignment to const/var (already covered by spec — should use named function declaration), return statement (anonymous returned function), IIFE (already covered by `FORBIDDEN_IIFE`). Drift attaches to the `FILE_HEADER` row.
   - **CSS pre-load reporting:** when `Get-SqlData` returns 0 CSS_CLASS DEFINITION rows OR returns null (query failure), emit a banner-style warning at the END of the run summary so it's the last thing the operator sees, not buried mid-run. Format: structured key/value lines that the Admin UI can parse plus a human-readable banner. Both conditions trigger the same end-of-run signal — empty result set is the post-truncate-no-CSS-yet scenario; null is the query-failure scenario. The mid-run WARN line stays in place for immediate signal during interactive runs.
   - **Trim removal:** every `Limit-Text` call removed from the populator. The `Limit-Text` and `Format-SingleLine` helpers retire if they become unused. Schema columns widened (`component_name` → VARCHAR(500), `variant_qualifier_2` → VARCHAR(500), `source_section` → VARCHAR(300)) so the populator never needs to truncate. `drift_codes` stays at VARCHAR(500); `parent_function` stays at VARCHAR(200); `variant_type` stays at VARCHAR(30); `variant_qualifier_1` stays at VARCHAR(100). Free-form text columns (`signature`, `raw_text`, `purpose_description`, `drift_text`) stay at VARCHAR(MAX). The populator becomes "what the parser found, verbatim."

   **Verification:** after the populator pass, the verification query results should show (a) variant columns populated for the relevant component types — non-zero count of rows where `variant_type IS NOT NULL`; (b) `parent_function` populated for all USAGE rows that fall inside a function body and for all `JS_METHOD` / `JS_METHOD_VARIANT` rows; (c) the column-fill audit confirms every column in the table receives at least one non-NULL value across the row set; (d) the run completes with a structured success/degraded/failed end-state.

   **Validation strategy:** because every existing JS file is non-spec-compliant, running the populator over the full codebase will produce thousands of drift rows that don't tell us anything we don't already know. The genuinely valuable validation is running the populator against the refactored shared file (`engine-events.js` → `cc-shared.js`) — where we expect zero drift. That's the test that tells us the populator and the spec agree. Phase 1 page-file refactors then iterate page by page; each refactored file is a fresh validation point that can surface spec/populator gaps.
