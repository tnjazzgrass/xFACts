...
**JS Phase 1 refactor work in progress.** Two of five Phase 1 page JS files have been refactored to spec compliance: `client-relations.js` and `business-services.js`. Both come back at zero drift on structural compliance. `business-services.js` carries 9 `FORBIDDEN_INLINE_EVENT_IN_JS` rows from inline `onclick="..."` attribute strings in template literals — these are spec-honest drift, not structural drift, and resolve via the inline event handler migration in the next session. The remaining three Phase 1 page JS files (`backup.js`, `replication-monitoring.js`, `business-intelligence.js`) are queued for the next session after the inline event cleanup completes.

**Inline event handler drift discovered and surfaced (2026-05-08).** During the `business-services.js` refactor, inline `onclick="..."` attribute strings inside template literals were observed to be widespread across the codebase (281 rows in unrefactored files, 9 in business-services.js, 0 in client-relations.js). The spec did not previously address these — it covered direct property assignment (`el.onclick = handler`) via `FORBIDDEN_PROPERTY_ASSIGN_EVENT` and inline `<script>`/`<style>` in template literals via `FORBIDDEN_INLINE_SCRIPT_IN_JS` / `FORBIDDEN_INLINE_STYLE_IN_JS`, but inline event-handler attributes fell through that gap. The spec was amended to forbid the pattern (§14, §16.2, §17, §18.4) and the JS populator was patched to detect it. The cleanup is queued as the first action item in the next session.

**Engine processes contract banner adopted (2026-05-08).** `CC_JS_Spec.md` §7.4 introduces a fixed-form `CONSTANTS: ENGINE PROCESSES` banner with `Prefix: (none)` for the `ENGINE_PROCESSES` page-to-platform contract identifier. Mirrors the §8 hooks pattern: contract surfaces between page files and `cc-shared.js` are structurally visible in the file via dedicated banners with `Prefix: (none)`, rather than via name-based prefix exemptions inside otherwise-prefixed sections.
...
- `CC_JS_Spec.md` — Production. As of 2026-05-08: spec hygiene principle added to preamble (specs describe rules and shapes, never present contents); §7.4 introduces the fixed-form `CONSTANTS: ENGINE PROCESSES` banner; §10 clarifies that an empty IMPORTS section omits its banner entirely; §14 / §16.2 / §17 / §18.4 add `FORBIDDEN_INLINE_EVENT_IN_JS` covering inline `on<event>="..."` attributes in template/string literals; §7.2 distinguishes SCREAMING_SNAKE_CASE for primitives from camelCase for objects/arrays/computed values; §8 hooks pattern preserved unchanged. As of 2026-05-07: §3.1 tightened to the 76-character banner rule (matching CSS); §3.2 permissive admission/strict validation paragraph; §5.4 added the `PREFIX_MISSING` rule for top-level identifiers; §14 added `FORBIDDEN_REVEALING_MODULE` row and a new §14.2 on forbidden wrapper patterns; §18.2 retired `MALFORMED_SECTION_BANNER` in favor of seven granular `BANNER_*` codes; §18.3 added `PREFIX_MISSING`; §18.4 added `FORBIDDEN_REVEALING_MODULE`. The spec and the populator are aligned; the populator emits exactly the drift codes the spec defines.
...
| `business-services.js` | `bsv` | PENDING — structural refactor done at zero drift; 9 `FORBIDDEN_INLINE_EVENT_IN_JS` rows pending inline event handler migration |
| `backup.js` | `bkp` | QUEUED for Phase 1 batch |
| `business-intelligence.js` | `biz` | QUEUED for Phase 1 batch (full rewrite — revealing-module file) |
...
1. **Inline event handler migration on `business-services.js`.** Resolve the 9 `FORBIDDEN_INLINE_EVENT_IN_JS` rows by replacing inline `onclick="..."` attributes in template literals with `addEventListener` bindings established after rendering. First action item of the next session. Closes the file at fully zero drift.
2. **Remaining Phase 1 JS refactors.** Three page JS files remain in the Phase 1 set:
   - `backup.js` — standard spec application, prefix `bkp`. Inline event handler count to be measured during refactor.
   - `replication-monitoring.js` — standard spec application, prefix `rpm`. Inline event handler count to be measured during refactor.
   - `business-intelligence.js` — **full file rewrite** (revealing-module file; in the FORBIDDEN_REVEALING_MODULE list). Restructure inner functions to top-level form with `biz_*` prefix; remove the wrapper namespace; update any external call sites that reference the wrapper namespace.

   Each refactored file is expected to surface inline event handler rows similar to business-services.js. The per-file migration to `addEventListener` happens in the same session as the structural refactor for that file, so each Phase 1 page lands at fully zero drift in a single pass rather than carrying pending drift forward.
...
3. HTML populator catch-up plus HTML spec design. Bring `Populate-AssetRegistry-HTML.ps1` to current schema, resolve the dynamic-class strategy question (see Pipeline doc), then design `CC_HTML_Spec.md` against existing HTML conventions. HTML populator should also be refactored to consume `xFACts-AssetRegistryFunctions.ps1` for consistency, and to adopt permissive-admission/strict-validation for any HTML constructs that have a defined shape (e.g., specific div role attributes). Inline event handlers on static HTML elements in route `.ps1` here-strings will be a separate cataloging concern at this stage — the JS populator's inline-event detection covers JS template/string literal cases only, not raw HTML markup in PowerShell here-strings.
4. PS populator plus PS spec design (modules and routes). Two specs, one populator covering both. Will also consume the helpers file and follow the permissive-admission pattern.
5. Phase 1 batch sweep — complete the Phase 1 pages across HTML and PS to bring the pages fully online. CSS is done (Phase 1 CSS files at zero drift); JS will be done after item 2 above; HTML and PS are the remaining halves.
