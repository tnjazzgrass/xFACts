...
**JS populator refactor delivered.** `Populate-AssetRegistry-JS.ps1` was refactored across the 2026-05-07 session. Same shape as the CSS work (consume helpers, visitor pattern via `Invoke-AstWalk`, pre-built section list, prefix registry validation with strict-with-carve-outs Option B, permissive-admission/strict-validation banner detection). Two JS-specific additions landed in the same refactor: detection of the revealing-module IIFE pattern (`const X = (function(){...})()` and var equivalent) with the new `FORBIDDEN_REVEALING_MODULE` drift code, and a `PREFIX_MISSING` drift code for top-level identifiers whose names don't begin with the file's registered `cc_prefix` (independent of banners; closes the gap where pre-spec files were silently exempt from prefix scrutiny). A definition-suppression flag mechanism (`$script:CurrentSuppressDefinitions`) was implemented for forbidden top-level wrapper patterns: when detected, the wrapper row emits with appropriate `FORBIDDEN_*` drift, then inner definition emissions are suppressed for the rest of the file while USAGE emissions continue, keeping the cross-reference catalog complete. Output validated against the full 25-file JS set: 9,639 rows / 20.9% drift, all files walking to completion. cc-shared.js continues at zero file-attributable drift. First empirical validation of the spec-compliant case beyond cc-shared.js will come with the first Phase 1 page JS file refactor.
...
| File | Wrapper pattern | Drift code |
|------|----------------|------------|
| `admin.js` | `const Admin = (function(){...})()` | `FORBIDDEN_REVEALING_MODULE` |
| `bdl-import.js` | `const BDL = (function(){...})()` | `FORBIDDEN_REVEALING_MODULE` |
| `applications-integration.js` | revealing-module | `FORBIDDEN_REVEALING_MODULE` |
| `client-portal.js` | revealing-module | `FORBIDDEN_REVEALING_MODULE` |
| `business-intelligence.js` | revealing-module | `FORBIDDEN_REVEALING_MODULE` |
| `platform-monitoring.js` | revealing-module | `FORBIDDEN_REVEALING_MODULE` |
| `ddl-erd.js` | top-level IIFE | `FORBIDDEN_IIFE` |
| `ddl-loader.js` | top-level IIFE | `FORBIDDEN_IIFE` |
| `docs-controlcenter.js` | top-level IIFE | `FORBIDDEN_IIFE` |
| `nav.js` | top-level IIFE | `FORBIDDEN_IIFE` |

These ten files require full rewrite rather than in-place repair. Their inner functions and constants live inside a wrapper namespace that has no equivalent in the spec'd top-level-function model; migrating any of them means restructuring every call site that references the wrapper namespace. Until they're rewritten, the catalog continues to flag the wrapper as forbidden via drift on the wrapper row, and the cross-reference USAGE rows from inside their bodies continue to populate the catalog so cross-page consumption queries during Phase 1 remain accurate. `business-intelligence.js` is in the Phase 1 set (will be rewritten during the Phase 1 batch sweep). The other nine are queued for the per-page migration phase.

**Next steps (queued work):**

The first three items below are the next-session pickup point. Items 1 and 2 are small drift-cleanup tasks that should land before the Phase 1 JS refactor in item 3 so the populator runs cleanly on the refactored files and any drift seen there is signal, not noise from known issues.

1. **CSS populator EXCESS_BLANK_LINES bug fix.** The populator's `EXCESS_BLANK_LINES` check is counting blank lines that occur *inside* the file header comment block as "between top-level constructs." Six files trip this: `cc-shared.css` and the five spec.css reference files (`backup-spec.css`, `business-intelligence-spec.css`, `business-services-spec.css`, `client-relations-spec.css`, `replication-monitoring-spec.css`). All six have multi-paragraph file headers; files with single-paragraph headers don't fire the false positive. Fix the detector to ignore content inside comment blocks. No spec change needed. Same fix closes the drift on all six files.

2. **CSS spec amendment + cc-shared.css cleanup for the remaining cc-shared drift.** Three categories of work bundled because they touch related areas:
   - **`BLANK_LINE_INSIDE_RULE` on `:root` block (cc-shared.css 49-192) — spec amendment.** The `BLANK_LINE_INSIDE_RULE` rule was written for class definitions; `:root` token blocks legitimately use blank lines as visual category separators. Amend `CC_CSS_Spec.md` to carve `:root` out of the rule, and update the populator to suppress the check inside `:root`. Decide on the cleanest carve-out form during the amendment.
   - **`FORBIDDEN_COMPOUND_DECLARATION` on three rules (cc-shared.css 217, 219, 223) — file edit.** The `::-webkit-scrollbar`, `::-webkit-scrollbar-thumb`, and `*` rules are written single-line with multiple declarations on the same line. Real violations of the rule as written. Reformat each so each declaration is on its own line.
   - **`FORBIDDEN_COMMENT_STYLE` at cc-shared.css lines 198 and 361 — file edit.** Two stray block comments not matching any of the four allowed kinds. Inspect each and either remove (moving content into the section banner description block) or reformat as sub-section markers (`/* -- label -- */`).

3. **Phase 1 JS refactor — bring the five page JS files into spec compliance.** Mirror of the Phase 1 CSS refactor work. Brings the five Phase 1 pages to "CSS and JS both fully spec-compliant, awaiting HTML and PS." Validates the JS populator against five additional real spec-compliant files beyond cc-shared.js. Files:
   - `backup.js` — standard spec application, prefix `bkp`.
   - `client-relations.js` — standard spec application, prefix `clr`.
   - `replication-monitoring.js` — standard spec application, prefix `rpm`.
   - `business-services.js` — standard spec application, prefix `bsv`.
   - `business-intelligence.js` — **full file rewrite** (revealing-module file; in the FORBIDDEN_REVEALING_MODULE list). Restructure inner functions to top-level form with `biz_*` prefix; remove the wrapper namespace; update any external call sites that reference `BizIntel.foo` (or whatever the wrapper namespace is named).

   The five pages remain inactive (still using their pre-spec runtime versions) until HTML and PS specs land and HTML/PS catch-up completes. This step gets the CSS and JS halves of those pages staged and ready.

4. HTML populator catch-up plus HTML spec design. Bring `Populate-AssetRegistry-HTML.ps1` to current schema, resolve the dynamic-class strategy question (see Pipeline doc), then design `CC_HTML_Spec.md` against existing HTML conventions. HTML populator should also be refactored to consume `xFACts-AssetRegistryFunctions.ps1` for consistency, and to adopt permissive-admission/strict-validation for any HTML constructs that have a defined shape (e.g., specific div role attributes).
5. PS populator plus PS spec design (modules and routes). Two specs, one populator covering both. Will also consume the helpers file and follow the permissive-admission pattern.
6. Phase 1 batch sweep — complete the Phase 1 pages across HTML and PS to bring the pages fully online. CSS is done (Phase 1 CSS files at zero drift); JS will be done after item 3 above; HTML and PS are the remaining halves.
7. **`docs-base.css` → `docs-shared.css` migration.** Parallel to the `engine-events.css` → `cc-shared.css` migration. Create `docs-shared.css` next to `docs-base.css`; migrate sections incrementally (FOUNDATION first — variables and resets; then chrome; then layout primitives) using the populator's SHARED/LOCAL scope columns to track progress; retire `docs-base.css` when empty; update `<link>` tags across all docs pages. The populator's anchor-file enforcement check (drift codes `DUPLICATE_FOUNDATION`, `DUPLICATE_CHROME`) will need to learn about the second anchor file — small change, lands alongside the first docs CSS file refactor when we get there.
8. **Per-file refactor for the seven docs CSS files** — joins the same queue as the unrefactored CC files. Same six-category treatment per file (banner conversion, prefix declarations, descendant-combinator resolutions, group selector splits, hex-to-token swaps, etc.). Note: `FORBIDDEN_DESCENDANT` resolutions require coordinated HTML updates across docs pages (`index.html`, `engine-room.html`, `teams.html`, `arch/*.html`, `cc/*.html`, `ref/*.html`); these fall under the same per-file refactor scope. `docs-erd.css` additionally requires a `ddl-erd.js` change to emit a single combined `.is-pk-fk` class (replacing the depth-3 compound `.erd-table-col.is-pk.is-fk`).
9. Page-at-a-time migration for the remaining ~22 CC pages.
...
The CSS and JS populators read this column at startup to validate banner-declared prefixes and to validate top-level identifier names against the file's registered prefix. See `CC_CSS_Spec.md` Section 5.4 and `CC_JS_Spec.md` Section 5.4 for the validation rules.
...
| `Documentation.Site` | `docs-shared.css` | Planned. Currently `docs-base.css` until the migration described in Queued Work item 4 completes. |
