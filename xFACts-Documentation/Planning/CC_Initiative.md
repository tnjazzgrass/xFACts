...
**§12 event handler binding amendment + populator catch-up (2026-05-09).** `CC_JS_Spec.md` §12 introduces a dedicated section on event handler binding. §12.1 establishes event delegation as the canonical form: a single `addEventListener` on a stable parent in INITIALIZATION, with the handler dispatching by `event.target.matches`/`closest` plus `data-*` attributes. §12.2 names the two permitted direct-binding cases (singleton elements bound at page boot, window-level/document-level events) and explicitly excludes a form-input carve-out. The amendment was prompted by the recurring pattern of `forEach + addEventListener` loops in the source files being refactored — this pattern attaches one listener per element rather than delegating, which fails on dynamically-injected content and creates listener leaks on re-render. New drift code `FORBIDDEN_PER_ELEMENT_LISTENER_LOOP` added to §15 forbidden patterns and §19.4. The JS populator was patched in the same session to detect the new code: a `Test-IsInsideElementLoop` helper walks the parent-node chain looking for an enclosing `forEach` (or sibling `map`/`filter`/`find`/`some`/`every`) callback or a `for...of` / `for...in` / `for` loop body, stopping at any nested `FunctionDeclaration` so inner functions don't false-positive their own listeners; when the check fires, the drift code attaches to the existing `JS_EVENT` USAGE row rather than emitting a new row. Spec and populator are aligned on every drift code.
...
3. Phase 1 batch sweep — complete the Phase 1 pages across HTML and PS to bring the pages fully online. CSS is done (Phase 1 CSS files at zero drift); JS is done (all four Phase 1 page JS files plus the rewritten `business-intelligence.js` at zero drift, delivered as offline `*-spec.js` companions); HTML and PS are the remaining halves. The flip for each page is coordinated: route HTML inline `onclick` references update to the renamed function names, route HTML `connection-error` elements migrate to the cc-shared `connection-banner` pattern, route HTML `<script>` tags add the cc-shared.js load alongside or in place of engine-events.js, and the page is tested as a unit before declaring it migrated.
