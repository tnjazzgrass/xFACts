...
**Status:** Active — **collection layer AND Control Center page LIVE**. Collection went live 2026-07-12; the B2B Pipeline page went live 2026-07-13. The second 2026-07-16 session (Step 07) **closed the dispatcher problem**: the run→schedule-link framing was disproven, ~142K rows were resolved via verified structural propagation + validated schedule-timing adjudication (~76% → ~84% coverage), 806 mislabeled rows were corrected, and the residual is fully characterized and tabled. Next: fault-report content completeness (time-sensitive), then the enrichment survey.
**Version:** 3.1
...
1. **🎯 Fault-report content completeness.** Dev-side comparison shows the IBM Status Report contains content the current `SI_FaultReport` capture does not fully carry — the parsed JSON / raw text stored per fault does not fully match what devs see in the IBM UI. **Time-sensitive:** the STATUS_RPT handle retention is ~3-4 days (§4.3), so anything not captured in-window is lost permanently. Work: pull recent faults and compare captured JSON/raw against the IBM Status Report side by side; identify what is missing (candidate causes to check: report shapes beyond TRANSLATION/SERVICE/MESSAGE, additional report-producing steps/handles per run, and blob-read truncation — verify `-MaxBinaryLength` on every TRANS_DATA read); adjust the collector's fault-report enrichment step accordingly. Follow-on (later, once content is right): restructure the run-detail UI to display the report in a nicer format.
2. **Enrichment survey** — deciding what additional Integration/b2bi data to capture into the mirror and where to draw the line. Working framework from 2026-07-16: capture facts not interpretations; capture what you filter/group/alert on, not lookup-only detail (fetch-on-demand otherwise, like the fault report); capture at the grain you use; and capture when the source ages out (else join on demand). Concrete target (Dirk, 2026-07-16): a per-run summary for the run-detail view — file names, client keys with record counts (success/failure split), total records, and Jira tickets tied to Sterling faults; the sources are row-level and real-time in Integration (for certain file types), so the concept is a collector-gathered per-run summary table feeding the UI. CLIENTS_PARAM (per-client key/value config) remains a candidate reference source but should almost certainly be join-on-demand, not copied in.
...
4. For page work: `CC_PS_Spec.md`, `CC_CSS_Spec.md`, `CC_JS_Spec.md`, `CC_HTML_Spec.md` and the Guidelines CC-page checklist.

### Session start prompt template

> Continuing the B2B module (fault-report content completeness / enrichment survey / page iteration). Cache-busted manifest URL: https://raw.githubusercontent.com/tnjazzgrass/xFACts/main/manifest.json?v=<value>
...
1. **🎯 Fault-report content completeness.** The lead item (see Next Session §1). Compare SI_FaultReport captured content against the IBM Status Report for recent faults; close the capture gap in the collector's fault-report enrichment step (check report shapes, additional handles/steps per run, blob-read truncation / `-MaxBinaryLength`); then restructure the run-detail UI display (later, once content is right).
2. **Enrichment survey.** Decide what additional Integration/b2bi data to capture into the mirror, using the capture/join-on-demand framework (Next Session §2). Concrete target: the per-run file/client/record/Jira summary for the run-detail view (Next Session §2). CLIENTS_PARAM is a candidate reference source — likely join-on-demand, not copied in.
