...
3. **`WorkingFiles/B2B_Investigation/Step_07_Dispatcher_Resolution/Step_07_Findings.md`** — the dispatcher reframe, verified rules, and residual characterization.
4. **`WorkingFiles/B2B_Investigation/Step_08_FaultReport_Content/Step_08_Findings.md`** — the fault-report content mechanism, escalated recovery, and the retention/width lessons.
5. For page work: `CC_PS_Spec.md`, `CC_CSS_Spec.md`, `CC_JS_Spec.md`, `CC_HTML_Spec.md` and the Guidelines CC-page checklist.

### Session start prompt template

> Continuing the B2B module (enrichment survey — per-run summary / page iteration). Cache-busted manifest URL: https://raw.githubusercontent.com/tnjazzgrass/xFACts/main/manifest.json?v=<value>
...
- Dispatcher residual (tabled, Step 07): 269,661 NULLs = 44,917 scheduled parents (no-child sweeps + 824 unresolved conflicts), 123,017 children under them, 88,752 pre-2023-12-01 children whose parents never existed at the source (structurally unrecoverable), and 12,987 seq'd orphans. Optional last-resort ideas live in §9 item 8.
...
**2026-07-16 — Dispatcher resolution is run→definition, not run→schedule** — source: Collect-B2BPipeline.ps1 Step-b2b_ResolveDispatcherNames (Step 07)
The live path resolves `COALESCE(parent_id, run_id)` in `WF_INST_S` (+ `_RESTORE`) joined to `WFD` on (WFD_ID, WFD_VERSION) and takes the definition NAME; `SCHEDULE` is never consulted. A scheduled parent's dispatcher is its own workflow definition name (self-naming) — it equals the firing schedule's `service_name` only because that is the workflow the schedule launches. Coverage corollary (query returned empty): every live-resolved dispatcher value exists as a `service_name` in SI_ScheduleRegistry. The historical gap was never a missing schedule key; it is the purge of the run→definition mapping when instance rows age out.

**2026-07-16 — Verified rule: a child's dispatcher equals its parent's; parents self-name** — source: Step 07 (6,201 live parent/child pairs, both sides resolved from b2bi independently, zero mismatches)
Upgraded from design assumption to verified fact. Licensed upward propagation (resolved children name their NULL parent) and the correction of children contradicting a known parent value.

**2026-07-16 — The single-match sibling backfill produced false positives in sibling-schedule conflict families** — source: Step 07
1,548 NULL parents had resolved children that *disagreed* — impossible under the verified rule, so at least one value per parent was wrong. The signatures resolved to paired sibling schedules serving the same clients (HSS_HB EOBD/EOBD_2, ACADIA EO/EO_FD/EO_P2S_RC (+RTNT), HSS INSURANCE D2S_RC/NB_TR, LEXIS_NEXIS SP/SP2, MEDICAL CENTER NB/RT, REVSPRING BD_PULL/IB_PULL, a REVSPRING ITS 5-way, a Vanderbilt pair). Mechanism: when only one family member appeared in the live reference window, the single-match test passed and mislabeled historical rows actually dispatched by the other member. 806 provably-wrong children were corrected in the Step 07 pass; of 13,988 "unanimous but exposed" parents, only 13 were overruled by timing — the false-positive fear was real but small, and is now measured.

**2026-07-16 — Schedule-timing disambiguation validated and adopted (lag ≤ 2 min, no ties)** — source: Step 07 calibration + blind validation against live ground truth
92% of live scheduled runs land within 1 minute of a declared SI_ScheduleRegistry fire time (the 16+ minute tail is off-schedule launches). Blind validation within conflict families (known dispatcher hidden, smallest lag picks among family candidates): 147 correct / 4 incorrect / 0 ties — all four misses off-schedule runs the threshold rejects. Adopted: accept a timing pick only at lag ≤ 2 minutes with no tie; candidates without explicit fire times cannot win by construction; day-of-week masks ignored (validated as-is). **Decision (recorded):** timing-resolved values are inference, accepted into dispatcher_name without a marker column — the field is informational (not a reporting touchpoint), the go-forward path is exact, and accuracy measured ~100% under threshold. This bounds and supersedes the earlier "never write inference into dispatcher_name" caution for this field only; the general investigation-first stance is unchanged.

**2026-07-16 — Step 07 backfill: 104,022 parents + 38,005 children + 249 sibling rows resolved; 806 corrected; coverage ~76% → ~84%** — source: Step_07_Dispatcher_Backfill.sql against production
Resolution paths over the 104,846 NULL scheduled parents with resolved-child evidence: A-STRUCTURAL 89,310 (unanimous children outside conflict families), B-TIMING 14,507, B-FALLBACK-UNANIMOUS 205, UNRESOLVED-CONFLICT 824 (left NULL). The residual sibling fill excluded conflict-family values from its reference set. Post-pass table-wide rule check: zero child/parent dispatcher disagreements. Residual 269,661 NULLs, fully characterized: 44,917 scheduled parents (no-child sweeps + the 824 conflicts), 123,017 children under them, 88,752 pre-boundary children (next entry), 12,987 seq'd orphans.

**2026-07-16 — Scheduled parent rows begin midday 2023-12-01; 88,752 earlier children are structurally unrecoverable** — source: Step 07 residual census
Surgical boundary: missing-parent children end 2023-12-01 12:05, the oldest scheduled parent row in the mirror is 2023-12-01 12:10, in-table-parent children begin 12:30 the same day. Top-level scheduled parent rows do not exist in BATCH_STATUS history before that boundary — the 88,752 pre-boundary children reference parent runs that never wrote a status row at the source (not a capture gap in the mirror; yearly ramp 2021: 3,917 / 2022: 19,806 / 2023: 65,029 reflects volume growth to the cutover). Mechanism hypothesis (unverified): a source-side wrapper/BPML change that day began inserting a parent BATCH_STATUS row for scheduled sweeps. No in-table method can name a parent that does not exist.

### 4.2b Fault-report capture and content (Step 08)

**2026-07-16 — The stored report was complete; the parser was lossy** — source: Step 08
raw_report_text always captured the full Sterling status report; the gap was that the TRANSLATION parser kept only five fields per entry and dropped the Info sub-block (block name, field name/number/**data**, raw block data, signature tag, iteration, location index) plus report metadata. This was a parsing problem, not a capture/retention problem — so it was fixable retroactively by re-parsing from stored raw text. The parser now captures full per-entry Info detail into named fields, preserves unrecognized Info codes in additionalInfo, lifts report metadata to the payload top level, and the SERVICE shape captures every error line. A one-time re-parse (Reparse-B2BFaultReports.ps1) upgraded all existing rows.

**2026-07-16 — Escalated faults: the map succeeds, the BPML raises the exception** — source: Step 08 (run 8608465, probe-verified)
A large class of faults captured as one-line MESSAGE rows are not translation failures: the map completes with a warning-bearing report (Contains errors ? false), then the BPML inspects the outcome and raises a business exception (BPExceptionService, ADV_STATUS 25). The failing step's own report is a generic one-liner ("Unrecognized Data Block detected in translator..."); the *real* report — the one Melissa reads in Instance Data, with the Raw Block Data and Location Index — rides on the run's **last successful Translation step** (BASIC_STATUS = 0, SERVICE_NAME = 'Translation'), and is byte-identical to the copy embedded in the error step's CONTENT process-data blob. The collector now falls back to that successful-step report when the failing step yields only a MESSAGE, captures it as **TRANSLATION_ESCALATED**, and preserves the one-liner in SI_FaultReport.escalation_message. CONTENT scraping was evaluated and rejected: same report, but wrapped in Java serialization with no clean end boundary, versus the STATUS_RPT copy which is the exact format the parser already speaks.

**2026-07-16 — Probe findings: no pagination, no truncation, hard retention wall** — source: Step_08_Probe.ps1 across 349 recent handles
...
**Status:** ✅ Resolved (Step 6C census + Step 07); dispatcher_name ~84% populated; residual characterized and tabled
...
**Dispatch model + backfills (2026-07-16, see §4.2a and Step_07_Findings.md):** the two-mechanism arrival-time-routed dispatch model is verified from production data. dispatcher_name was backfilled in two same-day waves: (1) the structural backfill — ~1.3M NULLs via the GET_LIST parent/child chain and single-match sibling lookup; (2) **Step 07**, which disproved the run→schedule-link framing (live resolution is run→definition; scheduled parents self-name), verified child = parent (6,201 live pairs, zero mismatches), discovered and corrected 806 sibling-lookup false positives in sibling-schedule conflict families, validated schedule-timing adjudication on ground truth (147/4/0; adopted at lag ≤ 2 min, no ties), and resolved another ~142K rows to reach ~84% coverage. Residual: 269,661 rows, fully characterized (Next Session minor items). CLIENTS_PARAM's TRANSLATION_MAP was evaluated and rejected as a *direct* dispatcher source (it names the execution/translation service, not the dispatcher, and disagreed with trusted resolved rows); a fuzzy-match variant over the residual was re-floated (Dirk, Step 07 session) as an optional last-resort experiment — tabled, expectations low given that evaluation.
...
8. **Dispatcher residual (tabled).** 269,661 NULLs, fully characterized (§4.2a, Step 07). Optional last resorts, all low priority: CLIENTS_PARAM fuzzy-match experiment (TRANSLATION_MAP previously rejected as a *direct* source — expectations low; would need its own accuracy validation before any write), periodic sibling re-runs (near-exhausted: 249 rows on 2026-07-16), rare-cadence trickle-in. Accurate go-forward is the accepted bar.
